...
import 'package:tiki/utils/permission/go_live_permission.dart';
    ...
    return InkWell(
      onTap: () {
        if (liveModel.getMode == LiveStreamingModel.keyModePublic)
          LivePermissionHandler.checkPermission(
              liveModel.getStreamingType, context,
              liveStreamingModel: liveModel);
        else if (liveModel.getMode ==
            LiveStreamingModel.keyModeSubscribers) if (Get.find<
                    SubscriptionViewModel>()
                .isStreamerSubscribed(liveModel.getAuthor!) ==
            true)
          LivePermissionHandler.checkPermission(
              liveModel.getStreamingType, context,
              liveStreamingModel: liveModel);
          ...
          QuickHelp.showAppNotificationAdvanced(
              title:
                  "Subscribers only. Please subscribe to join the live stream!",
              context: context);
      },
      child: Container(
        ...
        child: Column(
          ...
          children: [
            if (liveModel.getAuthor!.getHideMyLocation == false &&
                (liveModel.getAuthor?.getCountry?.isNotEmpty ?? false))
              Row(
                children: [
                  Container(
              ...
              child: Row(
                children: [
                  ...
                  GestureDetector(
                    onTap: () {
                      goToProfile(
                          otherProfile: true, mUser: liveModel.getAuthor);
                    },
                    child: Container(
                      height: 40.h,
                      width: 40.w,
