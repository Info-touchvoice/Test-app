...
import '../trending/create/post_reel_screen.dart';
...
class _ReelsHomeScreenState extends State<ReelsHomeScreen>
  ...
  final CommunityController<VideoInfo> communityController =
      Get.find<CommunityController<VideoInfo>>();
  ...
  Future<List<VideoInfo>> _loadFeedsVideos(bool? isExclusive,
    ...
    if (widget.post != null) {
      ...
      queryBuilder.whereNotContainedIn(PostsModel.keyAuthor,
          Get.find<UserViewModel>().currentUser.getBlockedUsers!);
      queryBuilder.whereNotContainedIn(PostsModel.keyObjectId,
          Get.find<UserViewModel>().currentUser.getReportedPostIDs!);

      queryBuilder.whereDoesNotMatchQuery(PostsModel.keyAuthor, queryUsers);
      // Pagination: fetch small initial page fast, then load more on demand.
      queryBuilder.setAmountToSkip(skip ?? 0);
    }

    // Only include essential relation for faster initial load
    queryBuilder.includeObject([
      PostsModel.keyAuthor,
    ]);

    // Keep initial load lightweight for faster first-play.
