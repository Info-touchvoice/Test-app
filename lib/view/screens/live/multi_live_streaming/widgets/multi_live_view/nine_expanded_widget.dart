...
import 'package:tiki/view/screens/live/multi_live_streaming/widgets/zego_multi_live_view.dart';
...
class NineExpandedMultiGuestView extends StatelessWidget {
                            ...
                            if (coHostList.length >= gridController.seat!)
                              Expanded(
                                  child: ZegoMultiLiveView(
                                      userInfo: gridController.seat == 0
                                          ? host
                                          : coHostList[
                                              gridController.seat! - 1],
                                      seat: gridController.seat!)),
                            if (coHostList.length < gridController.seat!)
                              Expanded(
                                child: Container(
                                  ...
                                  child: Center(
                                      child: Text(
                                    'The co-host has left the session.',
                      ...
                      Expanded(
                        child: Column(
                          ...
                          children: [
                            ...
                            Expanded(
                              child: getUser(5, coHostList, host),
                            ),
                            Expanded(
                              child: getUser(7, coHostList, host),
                            ),
                          ],
                        ),
                      ),
                      Expanded(
                        child: Column(
                          children: [
                            ...
                            Expanded(
                              child: getUser(6, coHostList, host),
                            ),
                            Expanded(
                              child: getUser(8, coHostList, host),
                            ),
                          ],
                        ),
                      ),
  ...
  Widget getUser(int index, List<ZegoSDKUser> coHostList, ZegoSDKUser? host) {
    if (gridController.seat == index) {
      ...
      return ZegoMultiLiveView(
        userInfo: coHostList.length >= index ? coHostList[index - 1] : null,
        seat: index,
      );
  }
}
