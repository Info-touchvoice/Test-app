...
import 'package:tiki/services/room_action_records_logger.dart';
...
class _GuestsWidgetState extends State<GuestsWidget> {
  ...
  void initState() {
    if (zegoController.streamingType == LiveStreamingModel.keyTypeAudioLive) {
      final seatCount = Get.find<LiveViewModel>()
              .liveStreamingModel
              .getAudioSeats ??
          8;
      for (int index = 0; index < seatCount; index++) {
        ZegoLiveAudioRoomSeat seat =
            zegoController.getRoomSeatWithIndex(index);
...
class GuestOption extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final isAudio =
        zegoController.streamingType == LiveStreamingModel.keyTypeAudioLive;
    return Padding(
        ...
        children: [
          ...
          if (isAudio) ...[
            PrimaryButton(
              ...
              onTap: () async {
                Navigator.pop(context);
                await ZegoLiveAudioRoomManager.instance
                    .removeSpeakerFromSeat(targetUser.userID);
                await RoomActionRecordsLogger.logKickOutMic(targetUser);
              },
            ),
            Container(
              width: 342.w,
            ...
            PrimaryButton(
              ...
              onTap: () async {
                Navigator.pop(context);
                final uid = int.tryParse(targetUser.userID);
                if (uid != null) {
                  Get.find<LiveViewModel>().addKickOutUser(uid);
                }
                await ZegoLiveAudioRoomManager.instance
                    .kickOutRoom(targetUser.userID);
                await RoomActionRecordsLogger.logKickOutRoom(targetUser);
              },
            ),
          ] else
            PrimaryButton(
              ...
              onTap: () {
                Navigator.pop(context);
                zegoController.liveStreamingManager
                    .kickOutRoom(targetUser.userID.toString());
              },
            ),
          SizedBox(
            height: 20.h,
