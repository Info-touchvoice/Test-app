...
class ExpressService {
  ...
  Future<ZegoRoomLogoutResult> logoutRoom([String roomID = '']) async {
    final leaveResult = await ZegoExpressEngine.instance.logoutRoom(
        roomID.isNotEmpty ? roomID : currentRoomID);
  ...
  void enableVideoMirroring(bool isVideoMirror) {
    ZegoExpressEngine.instance.setVideoMirrorMode(
      isVideoMirror ? ZegoVideoMirrorMode.BothMirror : ZegoVideoMirrorMode
          .NoMirror,
    );
  }
  ...
  Future<void> startPlayingStream(String streamID,
      {ZegoViewMode viewMode = ZegoViewMode
  ...
  final roomUserListUpdateStreamCtrl = StreamController<
      ZegoRoomUserListUpdateEvent>.broadcast();
  final streamListUpdateStreamCtrl = StreamController<
      ZegoRoomStreamListUpdateEvent>.broadcast();
  final roomStreamExtraInfoStreamCtrl = StreamController<
      ZegoRoomStreamExtraInfoEvent>.broadcast();
  final roomStateChangedStreamCtrl = StreamController<
      ZegoRoomStateEvent>.broadcast();
  final roomExtraInfoUpdateCtrl = StreamController<
      ZegoRoomExtraInfoEvent>.broadcast();
  final recvAudioFirstFrameCtrl = StreamController<
      ZegoRecvAudioFirstFrameEvent>.broadcast();
  final recvVideoFirstFrameCtrl = StreamController<
      ZegoRecvVideoFirstFrameEvent>.broadcast();
  final recvSEICtrl = StreamController<ZegoRecvSEIEvent>.broadcast();
  final mixerSoundLevelUpdateCtrl = StreamController<
      ZegoMixerSoundLevelUpdateEvent>.broadcast();
  ...
  void initEventHandle() {
    ZegoExpressEngine.onRoomStreamUpdate =
        ExpressService.instance.onRoomStreamUpdate;
    ZegoExpressEngine.onRoomUserUpdate =
        ExpressService.instance.onRoomUserUpdate;
    ZegoExpressEngine.onRoomStreamExtraInfoUpdate =
        ExpressService.instance.onRoomStreamExtraInfoUpdate;
    ZegoExpressEngine.onRoomStateChanged =
        ExpressService.instance.onRoomStateChanged;
    // Use explicit lambdas for compatibility with older Dart versions.
    ZegoExpressEngine.onCapturedSoundLevelUpdate =
        (double soundLevel) async => ExpressService.instance.onCapturedSoundLevelUpdate(soundLevel);
    ZegoExpressEngine.onRemoteSoundLevelUpdate =
        (Map<String, double> soundLevels) async => ExpressService.instance.onRemoteSoundLevelUpdate(soundLevels);
    ZegoExpressEngine.onMixerSoundLevelUpdate =
        ExpressService.instance.onMixerSoundLevelUpdate;
    ZegoExpressEngine.onPlayerRecvAudioFirstFrame =
        ExpressService.instance.onPlayerRecvAudioFirstFrame;
    ZegoExpressEngine.onPlayerRecvVideoFirstFrame =
        ExpressService.instance.onPlayerRecvVideoFirstFrame;
    ZegoExpressEngine.onPlayerRecvSEI = ExpressService.instance.onPlayerRecvSEI;
    ZegoExpressEngine.onRoomExtraInfoUpdate =
        ExpressService.instance.onRoomExtraInfoUpdate;
  }

  void onRoomUserUpdate(String roomID,
    ...
    if (updateType == ZegoUpdateType.Add) {
      for (final user in userList) {
        ...
        if (userInfo == null) {
          userInfoList.add(
              ZegoSDKUser(userID: user.userID, userName: user.userName));
        } else {
          userInfo
            ..userID = user.userID
