...
extension ZIMServiceRoomMessage on ZIMService {
  Future<RoomRequestResult> sendRoomRequest(
    ...
    final request = RoomRequest(
        RoomRequestAction.request, currentZimUserInfo!.userID, receiverID,currentZimUserInfo!.userName )
  ...
  Future<RoomRequestResult> acceptRoomRequest(String requestID,
    ...
    print('newRoomRequest$newRoomRequest');
    // print('newRoomRequest${newRoomRequest['actionType']}');
    ...
    print({"result of sending command by broadcaster $result"});
    return result;
  }
  ...
  void onReceiveRoomMessage(
    ...
    for (final element in messageList) {
      if (element is ZIMCommandMessage) {
        ...
        debugPrint('onReceiveRoomCustomCommand: $messageMap');
          ...
          if (currentZimUserInfo!.userID == receiver) {
            switch (actionType) {
              case RoomRequestAction.request:
                final request = RoomRequest(actionType, sender, receiver, currentZimUserInfo!.userName);
                request.extendedData = extendedData;
                request.requestID = requestID;
                print("after checking request action ${request.senderID}");
                print("after checking request action ${request.requestID}");


                roomRequestMapNoti.addValue(requestID, request);
                print("roomRequestMapNoti${roomRequestMapNoti.value}");

                onInComingRoomRequestStreamCtrl.add(
                    OnInComingRoomRequestReceivedEvent(
                        requestID: requestID, extendedData: extendedData, request: request));
                break;
