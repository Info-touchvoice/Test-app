...
extension ExpressServiceStream on ExpressService {
  Future<void> stopPlayingStream(String streamID) async {
    ...
    if (isScreenSharingStream) {
      ...
      if (userInfo != null) {
        ...
        userInfo.viewID = -1;
        userInfo.soundLevelNotifier.value = 0.0;
      }
  ...
  Future<void> onCapturedSoundLevelUpdate(double soundLevel) async {
    // Local user's mic sound level.
    currentUser?.soundLevelNotifier.value = soundLevel;
  }

  Future<void> onRemoteSoundLevelUpdate(Map<String, double> soundLevels) async {
    // Remote users' sound levels keyed by streamID.
    // Map streamID -> userID using streamMap, then update that user's notifier.
    soundLevels.forEach((streamID, level) {
      final userID = streamMap[streamID];
      if (userID == null || userID.isEmpty) return;
      final user = getUser(userID);
      if (user == null) return;
      user.soundLevelNotifier.value = level;
    });
  }
