...
class LiveStreamingModel extends ParseObject implements ParseCloneable {
  ...
  int _safeInt(dynamic value) {
    ...
    // Parse SDK can keep unsaved ops as a Map, e.g. {"__op":"Increment","amount":1}
    if (value is Map) {
      final dynamic amount =
          value['amount'] ?? value['increment'] ?? value['value'];
  ...
  int? get getViewersCount {
    return _safeInt(get<dynamic>(keyViewersCountLive));
  }
  ...
  int? get getViewsCount => _safeInt(get<dynamic>(keyViewsCount));
  set setViewsCount(int count) => setIncrement(keyViewsCount, count);

  int? get getFansCount => _safeInt(get<dynamic>(keyFansCount));
  set setFansCount(int count) => setIncrement(keyFansCount, count);

  int? get getGifterCount => _safeInt(get<dynamic>(keyGifter));
  ...
  int? get getSubscriberGain => _safeInt(get<dynamic>(keySubscriberGain));
  set setSubscriberGain(int count) => setIncrement(keySubscriberGain, count);

  int? get getComments => _safeInt(get<dynamic>(keyComments));
