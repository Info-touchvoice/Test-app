...
import 'package:tiki/utils/helper/string_helper.dart';
...
class UserModel extends ParseUser implements ParseCloneable {
  ...
  static final String keyGenderOther = "other";
  ...
  static final String keyWebsite = "website";
  ...
  static final String keyEducation = "education";
  static final String keyCareer = "career";
  ...
  static final String keyVerificationExpiredAt = "verificationExpiredAt";
  ...
  static final String keyVIP = "vip";
  static final String keyLastSubscriptionDate = "vipSubscriptionDate";
  ...
  static final String keyBeans = "beans";
  static final String keyGold = "gold";
  ...
  static String keyIsLive = "isLive";
  static String keyOg = "og";
  ...
  String? get getFullName {
    ...
    if (name != null) {
      return StringHelper.capitalizeEachWord(name);
    } else {
      return getUsername;
    }
  }
  ...
  String? get getFirstName => getFullName?.split(" ")[0].capitalizeFirst ?? '';

  set setFirstName(String firstName) => set<String>(keyFirstName, firstName);

  String? get getLastName => getFullName?.split(" ")[1]?.capitalizeFirst ?? '';
  ...
  set setWebsite(String link) => set<String>(keyWebsite, link);
  ...
  set setAvatar7(ParseFileBase parseFileBase) =>
      set<ParseFileBase>(keyAvatar7, parseFileBase);

  ParseFileBase? get getAvatar8 => get<ParseFileBase>(keyAvatar8);
  set setAvatar8(ParseFileBase parseFileBase) =>
      set<ParseFileBase>(keyAvatar8, parseFileBase);
  ...
  DateTime? get getVipSubscriptionDate =>
      get<DateTime>(keyLastSubscriptionDate);
  set setVipSubscriptionDate(DateTime date) =>
      set<DateTime>(keyLastSubscriptionDate, date);
  ...
  bool get getIsAvatarSelected {
    bool? isSelected = get<bool>(keyIsAvatarSelected);
    if (isSelected == null) {
      return false;
    } else {
      return isSelected;
  ...
  set setAvatarSelected(bool selected) =>
      set<bool>(keyIsAvatarSelected, selected);

  String? get getAvatarId => get<String>(keySelectedAvatarId);
  set setAvatarId(String id) => set<String>(keySelectedAvatarId, id);

  bool? get getIsVip {
    bool? isVip = get<bool>(keyVIP);
    if (isVip == null) {
      return false;
    } else {
      return isVip;
  ...
  set setIsVip(bool isVip) => set<bool>(keyVIP, isVip);
  ...
  set verifyUser(bool verified) => set<bool>(keyUserVerified, verified);
  ...
  DateTime? get verificationExpiredDate {
    DateTime? verifiedAt = get<DateTime>(keyVerificationExpiredAt);
    return verifiedAt;
  }

  set verificationExpiredDate(DateTime? verifiedAt) =>
      set<DateTime?>(keyVerificationExpiredAt, verifiedAt);
  ...
  set setRelationship(String relationship) =>
      set<String>(keyRelationshipStatus, relationship);
  ...
  set setRelationshipStatus(String relationship) =>
      set<String>(keyRelationshipStatus, relationship);
  ...
  List<dynamic>? get getPreference {
    List<dynamic> preferences = [];

    List<dynamic>? preference = get<List<dynamic>>(keyPreference);
  ...
  set setPreference(List preference) => set<List>(keyPreference, preference);

  List<dynamic>? get getEducation {
    List<dynamic> educations = [];

    List<dynamic>? education = get<List<dynamic>>(keyEducation);
  ...
  set setEducation(List<Map<String, dynamic>> education) =>
      set<List<Map<String, dynamic>>>(keyEducation, education);

  List<dynamic>? get getCareer {
    List<dynamic> careers = [];

    List<dynamic>? career = get<List<dynamic>>(keyCareer);
  ...
  set setCareer(List<Map<String, dynamic>> career) =>
      set<List<Map<String, dynamic>>>(keyCareer, career);

  List<dynamic>? get getInterest {
    List<dynamic> interests = [];

    List<dynamic>? interest = get<List<dynamic>>(keyInterest);
  ...
  set setInterest(List interest) => set<List>(keyInterest, interest);
  ...
  set setHideMyBirthday(bool hideMyBirthday) =>
      set<bool>(keyHideBirthday, hideMyBirthday);
  ...
  int get getCoins {
    final dynamic token = get<dynamic>(keyCoinsTotal);
    if (token == null) return 0;

    // Parse "Number" fields may come back as int or double depending on SDK/serialization.
    if (token is int) return token;
    if (token is num) return token.toInt();
    if (token is String) return int.tryParse(token) ?? 0;

    return 0;
  }

  set setCoins(int diamondsTotal) => setIncrement(keyCoinsTotal, diamondsTotal);
  set decrementCoins(int diamondsTotal) =>
      setDecrement(keyCoinsTotal, diamondsTotal);

  int get getBeans => _readIntField(keyBeans);
  set setBeans(int amount) => setIncrement(keyBeans, amount);
  set decrementBeans(int amount) => setDecrement(keyBeans, amount);

  int get getGold => _readIntField(keyGold);
  set setGold(int amount) => setIncrement(keyGold, amount);
  set decrementGold(int amount) => setDecrement(keyGold, amount);

  int _readIntField(String key) {
    final dynamic token = get<dynamic>(key);
    if (token == null) return 0;
    if (token is int) return token;
    if (token is num) return token.toInt();
    if (token is String) return int.tryParse(token) ?? 0;
  ...
  List? get getFollowing {
    try {
      var retrievedData = get(keyFollowing);

      if (retrievedData is List) {
        List<dynamic> following = retrievedData;
        ...
        // Handle unexpected data types or inconsistencies gracefully
        return []; // Or handle the unexpected case appropriately
      }
    } catch (e) {
      print("Error retrieving data: $e");
      return []; // Handle errors by returning a default value or appropriate response
    }
  ...
  set removeAllFollowers(List followerList) =>
      setRemoveAll(keyFollowers, followerList);
  ...
  set setXp(int xp) => set<int>(keyXP, xp);

  set incrementLevel(int level) => setIncrement(keyLevel, level);
  set incrementXp(int xp) => setIncrement(keyXP, xp);
  ...
  String? get getSubscribeAnnouncement =>
      get<String>(keySubscriptionAnnouncement);
  set setSubscriptionAnnouncement(String announcement) =>
      set<String>(keySubscriptionAnnouncement, announcement);

  bool? get userIsLive => get<bool>(keyIsLive);
  set setUserLive(bool live) => set<bool>(keyIsLive, live);

  bool? get og => get<bool?>(keyOg);
  set og(bool? live) => set<bool?>(keyOg, live);
}
