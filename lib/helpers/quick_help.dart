...
import 'package:tiki/data/app/config.dart';
...
import 'package:tiki/utils/shared_manager.dart';
...
class QuickHelp {
  ...
  ///
  /// Priority:
  /// 1. If the user has explicitly selected/uploaded an avatar (via avatar
  ///    selection or profile upload), show that image.
  /// 2. Otherwise, show the gender‑based default avatar (male/female).
  static ImageProvider getUserAvatarProvider(UserModel? user) {
    if (user != null) {
      // Consider avatar "chosen" only if the user selected an avatar or has an
      // explicit avatarId saved (used for store avatars).
      final bool hasCustomAvatar =
          user.getIsAvatarSelected || ((user.getAvatarId ?? '').isNotEmpty);
      final String? url = hasCustomAvatar ? user.getAvatar?.url : null;
      ...
      if (gender != null && gender.toLowerCase() == 'female') {
        return const AssetImage(AppImagePath.defaultFemaleAvatar);
      }
    }

    // Fallback: use male default when nothing else is set
    return const AssetImage(AppImagePath.defaultMaleAvatar);
  }
  ...
  static Future<UserModel?> getUserAwait() async {
    ...
    try {
      final ParseResponse response = await currentUser
          .getUpdatedUser()
      ...
      if (errorCode == 101 || errorCode == 209) {
        await currentUser.logout(deleteLocalUserData: true);
        return null;
      }
    } catch (_) {
      // Offline or slow network: keep startup fast with cached user data.
    }
  ...
  static bool minimumAgeAllowed(
      String birthDateString, String datePattern, BuildContext context) {
  ...
  /// Pops a leftover modal from [showLoadingDialogWithText] when a tracked
  /// loading dialog was dismissed first.
  static void dismissUntrackedLoadingOverlay(BuildContext context) {
    final navigator = Navigator.of(context, rootNavigator: true);
          ...
          actions: [
            Column(
              children: [
                Padding(
                  padding:
                      const EdgeInsets.only(left: 17.0, right: 17, bottom: 5),
                  child: PrimaryButton(
  ...
  static void showAppNotificationAdvanced({
    ...
    showTopSnackBar(
      ...
      SnackBarPro.custom(
        backgroundColor:
            isError == null ? Colors.green : const Color(0xffff5252),
        title: title,
        message: message,
        icon: user != null
  ...
  static void showSnackBar({required String title}) {
    Get.snackbar(
      ...
      messageText:
          null, // Set messageText to null to remove the empty message text
    );
  }
              ...
              child: Center(
                child: Column(
                  ...
                  children: [
                    ...
                    useLogo ? SizedBox(height: 8.h) : const SizedBox(),
                    TextWithTap(
                      description,
  ...
  static Widget showLoadingAnimation(
      {Color leftDotColor = const Color(0xFFFFC107),
      Color rightDotColor = const Color(0xFFB71C1C),
      double size = 35}) {
    return Center(
      ...
      body: SizedBox(
        ...
        child: Container(
          ...
          child: Stack(
            children: [
              Positioned(
                // splashImage (legacy name removed)
                left: 0 * fem,
                top: 199 * fem,
                child: Align(
                  child: SizedBox(
  ...
  static Widget nothingIsHereCheer(double fem, double ffem,
      {double space = 100, double height = 200}) {
    return Column(
      children: [
  ...
  static dailyCheckInDialog(double fem, double ffem, BuildContext context,
      UserModel userModel, DateTime? dateTime,
      {Function? then}) {
    return showDialog(
      context: context,
  ...
  static congratulationOnReceivingXpDialog(BuildContext context, int xp,
      UserModel userModel, Function() onClickOnDone,
      {Function()? then, bool goBack = false, String? title}) {
    return showDialog(
        context: context,
        barrierDismissible: false,
        builder: (BuildContext context2) {
