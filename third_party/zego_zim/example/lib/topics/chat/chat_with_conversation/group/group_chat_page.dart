...
class GroupChatPage extends StatefulWidget {
  ...
  ZIMGroupFullInfo?
      myGroupFullInfo /* = ZIMGroupFullInfo(baseInfo: ZIMGroupInfo())*/;
...
class GroupChatPageState extends State<GroupChatPage> {
      ...
      body: Column(
        children: [
          ...
          MsgNormalBottomBox(
            ...
            onCameraIconButtonOnPressed: (path, [fileName]) {
              sendMediaMessage(ZIMMessageType.image, path, fileName);
            },
            onImageIconButtonOnPressed: (path, [fileName]) {
              sendMediaMessage(ZIMMessageType.image, path, fileName);
            },
            onVideoIconButtonOnPressed: (path, [fileName]) {
              sendMediaMessage(ZIMMessageType.video, path, fileName);
            },
          ),
        ],
      ),
  ...
  sendMediaMessage(
    ...
    ZIMMediaMessage mediaMessage = MsgConverter.mediaMessageFactory(
      path,
      messageType,
    );
    ...
    try {
      ...
      int index = widget._historyZIMMessageList.lastIndexWhere(
        (element) => element == mediaMessage,
      );
      Widget resultCell = MsgConverter.sendMediaMessageCellFactory(
        result.message as ZIMMediaMessage,
        null,
      );
      setState(() {
      ...
      for (ZIMMessage message in messageList) {
        switch (message.type) {
          case ZIMMessageType.text:
            ReceiveTextMsgCell cell = ReceiveTextMsgCell(
