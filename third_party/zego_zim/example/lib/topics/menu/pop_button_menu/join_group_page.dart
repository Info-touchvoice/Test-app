...
class _PageState extends State<JoinGroupPage> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: const Text('Join Group', style: TextStyle(color: Colors.black)),
            ...
            children: [
              ...
              ElevatedButton(
                onPressed: (() async {
                  FocusScope.of(context).requestFocus(FocusNode());
                  try {
                    ZIMGroupJoinedResult result =
                        await ZIM.getInstance()!.joinGroup(targetGroupID);
                    Navigator.pop(context);
                    Navigator.push(
                      context,
                      MaterialPageRoute(
                        builder: ((context) {
                          return GroupChatPage(
                            conversationID: result.groupInfo.baseInfo.groupID,
                            conversationName:
