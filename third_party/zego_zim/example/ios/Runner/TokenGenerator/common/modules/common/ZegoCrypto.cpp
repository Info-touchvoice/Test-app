...
void zego_aes_encrypt_cbc(char *plainText, uint32_t len, char *encrypted, BYTE key[],
                          uint32_t keyLength, BYTE iv[]) {
...
void zego_aes_decrypt_cbc(char *encrypted, uint32_t len, char *plainText, BYTE key[],
                          uint32_t keyLength, BYTE iv[]) {
    AES_WORD key_schedule[60] = {0};
    aes_key_setup(key, key_schedule, keyLength * 8);
...
void zego_aes_encrypt_ecb(char *plainText, uint32_t len, char *encrypted, BYTE key[],
                          uint32_t keyLength) {
    AES_WORD key_schedule[60] = {0};
    aes_key_setup(key, key_schedule, keyLength * 8);

    for (unsigned int i = 0; i < len / AES_BLOCK_SIZE; i++) {
        aes_encrypt((BYTE *)(plainText + i * AES_BLOCK_SIZE),
                    (BYTE *)(encrypted + i * AES_BLOCK_SIZE), key_schedule, keyLength * 8);
    }
}

void zego_aes_decrypt_ecb(char *encrypted, uint32_t len, char *plainText, BYTE key[],
                          uint32_t keyLength) {
    AES_WORD key_schedule[60] = {0};
    aes_key_setup(key, key_schedule, keyLength * 8);

    for (unsigned int i = 0; i < len / AES_BLOCK_SIZE; i++) {
        aes_decrypt((BYTE *)(encrypted + i * AES_BLOCK_SIZE),
                    (BYTE *)(plainText + i * AES_BLOCK_SIZE), key_schedule, keyLength * 8);
    }
}

std::string CZegoCrypto::AESEnc(const std::string &plainText, const std::string &key,
                                const std::string &iv) {
    ...
    zego_aes_encrypt_cbc((char *)padded.c_str(), padded.size(), (char *)encrypted.c_str(), m_Key,
                         key.size(), m_IV);
    return encrypted;
}

std::string CZegoCrypto::AESDec(std::string &encyptedText, const std::string &key,
                                const std::string &iv) {
    InitKV(key, iv);
    ...
    zego_aes_decrypt_cbc((char *)encyptedText.c_str(), encyptedText.size(),
                         (char *)decrypted.c_str(), m_Key, key.size(), m_IV);
    return decrypted;
}
...
std::string CZegoCrypto::AESEncECB(const std::string &plainText, const std::string &key) {
    ...
    zego_aes_encrypt_ecb((char *)padded.c_str(), padded.size(), (char *)encrypted.c_str(), m_Key,
                         key.size());
    return encrypted;
}
...
std::string CZegoCrypto::AESDecECB(std::string &encyptedText, const std::string &key) {
    ...
    zego_aes_decrypt_ecb((char *)encyptedText.c_str(), encyptedText.size(),
                         (char *)decrypted.c_str(), m_Key, key.size());
