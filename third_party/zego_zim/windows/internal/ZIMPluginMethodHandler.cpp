...
void ZIMPluginMethodHandler::callExperimentalAPI(FArgument &argument, FResult result) {
    CheckZIMInstanceExistAndObtainZIM();

    auto params = std::get<std::string>(argument[FTValue("params")]);

    zim->callExperimentalAPI(params, [=](const std::string &cb_result, const ZIMError &errorInfo) {
        if (errorInfo.code == 0) {
...
void ZIMPluginMethodHandler::updateUserCustomStatus(FArgument &argument, FResult result) {
    CheckZIMInstanceExistAndObtainZIM();

    auto customStatus = std::get<std::string>(argument[FTValue("customStatus")]);
    auto config = ZIMPluginConverter::oZIMUserCustomStatusUpdateConfig(
        std::get<FTMap>(argument[FTValue("config")]));

    zim->updateUserCustomStatus(
        customStatus, config, [=](const std::string &customStatus, const ZIMError &errorInfo) {
            if (errorInfo.code == 0) {
                FTMap retMap;
                retMap[FTValue("customStatus")] = FTValue(customStatus);
                result->Success(retMap);
            } else {
                result->Error(std::to_string(errorInfo.code), errorInfo.message);
            }
...
void ZIMPluginMethodHandler::updateUserBadge(FArgument &argument, FResult result) {
    CheckZIMInstanceExistAndObtainZIM();

    int badge = ZIMPluginConverter::cnvFTValueToInt32(argument[FTValue("badge")]);

    zim->updateUserBadge(badge, [=](const ZIMError &errorInfo) {
        if (errorInfo.code == 0) {
            result->Success();
        } else {
            result->Error(std::to_string(errorInfo.code), errorInfo.message);
...
void ZIMPluginMethodHandler::sendMessage(FArgument &argument, FResult result) {
    ...
    int32_t mediaUploadingCallbackID =
        ZIMPluginConverter::cnvFTValueToInt32(argument[FTValue("mediaUploadingCallbackID")]);
    int32_t multipleMediaUploadingCallbackID = ZIMPluginConverter::cnvFTValueToInt32(
        argument[FTValue("multipleMediaUploadingCallbackID")]);

    std::shared_ptr<ZIMPushConfig> pushConfigPtr = nullptr;
    std::shared_ptr<ZIMVoIPConfig> voIPConfigPtr = nullptr;
    std::shared_ptr<ZIMOfflinePushPrivateMessageTemplate> templatePtr = nullptr;
    auto config = ZIMPluginConverter::oZIMMessageSendConfig(
        std::get<FTMap>(argument[FTValue("config")]), pushConfigPtr, voIPConfigPtr, templatePtr);
    ...
    auto notification = std::make_shared<zim::ZIMMessageSendNotification>(
        ...
        [=](const std::shared_ptr<zim::ZIMMediaMessage> &message,
            ...
            progressRetMap[FTValue("mediaUploadingCallbackID")] = FTValue(mediaUploadingCallbackID);
            progressRetMap[FTValue("messageID")] = FTValue(messageID);
        ...
        [=](const std::shared_ptr<zim::ZIMMultipleMessage> &message,
            unsigned long long currentFileSize, unsigned long long totalFileSize,
            unsigned int messageInfoIndex, unsigned long long currentIndexFileSize,
            unsigned long long totalIndexFileSize) {
            FTMap multipleProgressRetMap;
            multipleProgressRetMap[FTValue("handle")] = FTValue(handle);
            multipleProgressRetMap[FTValue("method")] = FTValue("multipleUploadMediaProgress");
            multipleProgressRetMap[FTValue("multipleMediaUploadingCallbackID")] =
                FTValue(multipleMediaUploadingCallbackID);
            multipleProgressRetMap[FTValue("messageID")] = FTValue(messageID);
            multipleProgressRetMap[FTValue("message")] =
                ZIMPluginConverter::cnvZIMMessageObjectToMap(message.get());
            multipleProgressRetMap[FTValue("currentFileSize")] = FTValue((int64_t)currentFileSize);
            multipleProgressRetMap[FTValue("totalFileSize")] = FTValue((int64_t)totalFileSize);
            multipleProgressRetMap[FTValue("messageInfoIndex")] =
                FTValue((int32_t)messageInfoIndex);
            multipleProgressRetMap[FTValue("currentIndexFileSize")] =
                FTValue((int64_t)currentIndexFileSize);
            multipleProgressRetMap[FTValue("totalIndexFileSize")] =
                FTValue((int64_t)totalIndexFileSize);
            ZIMPluginEventHandler::getInstance()->sendEvent(multipleProgressRetMap);
        });
...
void ZIMPluginMethodHandler::sendMediaMessage(FArgument &argument, FResult result) {
    ...
    auto notification = std::make_shared<zim::ZIMMediaMessageSendNotification>(
        ...
        [=](const std::shared_ptr<zim::ZIMMediaMessage> &message,
            ...
            progressRetMap[FTValue("handle")] = FTValue(handle);
            progressRetMap[FTValue("method")] = FTValue("uploadMediaProgress");
            progressRetMap[FTValue("mediaUploadingCallbackID")] = FTValue(progressID);
            progressRetMap[FTValue("messageID")] = FTValue(messageID);
            progressRetMap[FTValue("message")] =
                ZIMPluginConverter::cnvZIMMessageObjectToMap(message.get());
            progressRetMap[FTValue("currentFileSize")] = FTValue((int64_t)currentFileSize);
...
void ZIMPluginMethodHandler::downloadMediaFile(FArgument &argument, FResult result) {
    ...
    auto config =
        ZIMPluginConverter::oZIMMediaDownloadConfig(std::get<FTMap>(argument[FTValue("config")]));

    zim->downloadMediaFile(
        std::static_pointer_cast<ZIMMessage>(mediaMessagePtr), (ZIMMediaFileType)fileType, config,
        [=](const std::shared_ptr<ZIMMessage> &message, unsigned long long currentFileSize,
    ...
    auto notification = std::make_shared<zim::ZIMMessageSendNotification>(
        ...
        [=](const std::shared_ptr<zim::ZIMMultipleMessage> &message,
            ...
            multipleProgressRetMap[FTValue("messageInfoIndex")] =
                FTValue((int32_t)messageInfoIndex);
            multipleProgressRetMap[FTValue("currentIndexFileSize")] =
                FTValue((int64_t)currentFileSize);
            multipleProgressRetMap[FTValue("totalIndexFileSize")] = FTValue((int64_t)totalFileSize);
            ZIMPluginEventHandler::getInstance()->sendEvent(multipleProgressRetMap);
        });
...
void ZIMPluginMethodHandler::editMessage(FArgument &argument, FResult result) {
    CheckZIMInstanceExistAndObtainZIM();
    ...
    auto editConfig =
        ZIMPluginConverter::oZIMMessageEditConfig(std::get<FTMap>(argument[FTValue("config")]));
    ...
    zim->editMessage(
        message, editConfig, notification,
        [=](const std::shared_ptr<ZIMMessage> &callbackMessage, const ZIMError &errorInfo) {
            FTMap retMap;
            auto messageMap = ZIMPluginConverter::cnvZIMMessageObjectToMap(callbackMessage.get());
            retMap[FTValue("message")] = messageMap;
            retMap[FTValue("messageID")] = FTValue(messageID);
...
void ZIMPluginMethodHandler::cancelSendingMessage(FArgument &argument, FResult result) {
    CheckZIMInstanceExistAndObtainZIM();

    auto message =
        ZIMPluginConverter::cnvZIMMessageToObject(std::get<FTMap>(argument[FTValue("message")]));
    auto cancelConfig = ZIMPluginConverter::oZIMSendingMessageCancelConfig(
        std::get<FTMap>(argument[FTValue("config")]));

    zim->cancelSendingMessage(message, cancelConfig, [=](ZIMError errorInfo) {
        if (errorInfo.code == 0) {
            result->Success();
        } else {
            result->Error(std::to_string(errorInfo.code), errorInfo.message);
...
void ZIMPluginMethodHandler::queryHistoryMessage(FArgument &argument, FResult result) {
    ...
    config.mode = static_cast<ZIMMessageQueryMode>(std::get<int32_t>(configMap[FTValue("mode")]));
...
void ZIMPluginMethodHandler::pinMessage(FArgument &argument, FResult result) {
    CheckZIMInstanceExistAndObtainZIM();

    auto message =
        ZIMPluginConverter::cnvZIMMessageToObject(std::get<FTMap>(argument[FTValue("message")]));
    auto isPinned = std::get<bool>(argument[FTValue("isPinned")]);

    ZIMMessagePinConfig config{};

    auto configMap = std::get<FTMap>(argument[FTValue("config")]);
    config.disableAutoReplace = std::get<bool>(configMap[FTValue("disableAutoReplace")]);

    zim->pinMessage(message, isPinned, config, [=](const ZIMError &errorInfo) {
        if (errorInfo.code == 0) {
            result->Success();
        } else {
            result->Error(std::to_string(errorInfo.code), errorInfo.message);
...
void ZIMPluginMethodHandler::queryPinnedMessageList(FArgument &argument, FResult result) {
    CheckZIMInstanceExistAndObtainZIM();
...
void ZIMPluginMethodHandler::leaveAllRoom(FArgument &argument, FResult result) {
    ...
    zim->leaveAllRoom([=](const std::vector<std::string> &roomIDList, const ZIMError &errorInfo) {
        if (errorInfo.code == 0) {
            FTMap retMap;
            retMap[FTValue("roomIDs")] = ZIMPluginConverter::cnvStlVectorToFTArray(roomIDList);
            result->Success(retMap);
        } else {
            result->Error(std::to_string(errorInfo.code), errorInfo.message);
        }
...
void ZIMPluginMethodHandler::createGroup(FArgument &argument, FResult result) {
    ...
    if (std::holds_alternative<std::monostate>(argument[FTValue("config")])) {
        zim->createGroup(
            groupInfo, userIDs,
...
void ZIMPluginMethodHandler::queryGroupList(FArgument &argument, FResult result) {
    ...
    if (std::holds_alternative<std::monostate>(argument[FTValue("count")])) {
        zim->queryGroupList([=](const std::vector<ZIMGroup> &groupList, long long nextFlag,
                                const ZIMError &errorInfo) {
            if (errorInfo.code == 0) {
                FTMap retMap;
                retMap[FTValue("groupList")] =
                    ZIMPluginConverter::cnvZIMGroupListToArray(groupList);
                retMap[FTValue("nextFlag")] = nextFlag;
                result->Success(retMap);
            } else {
                result->Error(std::to_string(errorInfo.code), errorInfo.message);
        ...
        int temp_count = ZIMPluginConverter::cnvFTValueToInt32(argument[FTValue("count")]);
        if (temp_count < 0) {
            temp_count = 0;
        }
        unsigned int count = temp_count;
        ZIMGroupListQueryConfig config;
        if (!std::holds_alternative<std::monostate>(argument[FTValue("config")])) {
            FTMap configMap = std::get<FTMap>(argument[FTValue("config")]);
            config.nextFlag = ZIMPluginConverter::cnvFTValueToInt64(configMap[FTValue("nextFlag")]);
            config.userIDs = ZIMPluginConverter::cnvFTArrayToStlVector(
                std::get<FTArray>(configMap[FTValue("userIDs")]));
        }
        zim->queryGroupList(count, config,
                            [=](const std::vector<ZIMGroup> &groupList, long long nextFlag,
                                const ZIMError &errorInfo) {
                                if (errorInfo.code == 0) {
                                    FTMap retMap;
                                    retMap[FTValue("groupList")] =
...
void ZIMPluginMethodHandler::addMessageReaction(FArgument &argument, FResult result) {
    ...
    auto config = ZIMPluginConverter::mZIMMessageReactionAddConfig(
        std::get<FTMap>(argument[FTValue("config")]));

    zim->addMessageReaction(reactionType, messagePtr, config,
...
void ZIMPluginMethodHandler::deleteMessageAllReactions(FArgument &argument, FResult result) {
...
void ZIMPluginMethodHandler::queryMessageReactionUserList(FArgument &argument, FResult result) {
    ...
    zim->queryMessageReactionUserList(
        ...
        [=](const std::shared_ptr<ZIMMessage> &message,
            const std::vector<ZIMMessageReactionUserFullInfo> &userList,
            const std::string &reactionType, const long long nextFlag,
            const unsigned int totalCount, const ZIMError &errorInfo) {
            if (errorInfo.code == 0) {
                FTMap retMap;
                retMap[FTValue("message")] =
                    ZIMPluginConverter::cnvZIMMessageObjectToMap(message.get());
                retMap[FTValue("userList")] =
                    ZIMPluginConverter::cnvZIMMessageReactionUserInfoListToArrayByUserFullInfoList(
                        userList);
                retMap[FTValue("userInfoList")] =
