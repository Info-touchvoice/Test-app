...
template struct Rob<ZIM_FriendlyGet_isServerMessage, &ZIMMessage::serverMessage>;

bool ZIMMessage::*get(ZIM_FriendlyGet_isGroupTargetedMessage);
template struct Rob<ZIM_FriendlyGet_isGroupTargetedMessage, &ZIMMessage::groupTargetedMessage>;
...
std::string ZIMMessage::*get(ZIM_FriendlyGet_editorUserID);
template struct Rob<ZIM_FriendlyGet_editorUserID, &ZIMMessage::editorUserID>;

long long ZIMMessage::*get(ZIM_FriendlyGet_editedTime);
template struct Rob<ZIM_FriendlyGet_editedTime, &ZIMMessage::editedTime>;

std::string ZIMMessage::*get(ZIM_FriendlyGet_pinnedUserID);
template struct Rob<ZIM_FriendlyGet_pinnedUserID, &ZIMMessage::pinnedUserID>;

long long ZIMMessage::*get(ZIM_FriendlyGet_pinnedTime);
template struct Rob<ZIM_FriendlyGet_pinnedTime, &ZIMMessage::pinnedTime>;
...
zim::ZIMConversationFilterOption ZIMPluginConverter::oZIMConversationFilterOption(FTMap ftmap) {
    ...
    option.pinnedFilter = (ZIMConversationPinnedFilter)ZIMPluginConverter::cnvFTValueToInt32(
        ftmap[FTValue("pinnedFilter")]);
    return option;
}
...
zim::ZIMMessageEditConfig ZIMPluginConverter::oZIMMessageEditConfig(FTMap ftmap) {
    zim::ZIMMessageEditConfig config;

    return config;
}

zim::ZIMSendingMessageCancelConfig ZIMPluginConverter::oZIMSendingMessageCancelConfig(FTMap ftmap) {
    zim::ZIMSendingMessageCancelConfig config;
...
FTMap ZIMPluginConverter::cnvZIMUserInfoObjectToMap(const ZIMUserInfo &userInfo) {
    ...
    userInfoMap[FTValue("userExtendedData")] = FTValue(userInfo.userExtendedData);
...
ZIMPluginConverter::cnvZIMUserInfoPtrToObj(const std::shared_ptr<ZIMUserInfo> userPtr) {
    ...
    userInfoMap[FTValue("userExtendedData")] = FTValue(userPtr->userExtendedData);
...
FTMap ZIMPluginConverter::cnvZIMConversationToMap(
    ...
    conversationMap[FTValue("pinnedTime")] = FTValue(conversation->pinnedTime);
    ...
    if (zim::ZIMConversationType::ZIM_CONVERSATION_TYPE_GROUP == conversation->type) {
        auto groupConv = std::static_pointer_cast<ZIMGroupConversation>(conversation);
        conversationMap[FTValue("mutedExpiredTime")] = FTValue(groupConv->mutedExpiredTime);
        conversationMap[FTValue("isDisabled")] = FTValue(groupConv->isDisabled);
    }
    conversationMap[FTValue("readMessageSeq")] = FTValue(conversation->readMessageSeq);
...
FTArray ZIMPluginConverter::cnvZIMMessagePinStatusChangeInfoListToArray(
    const std::vector<ZIMMessagePinStatusChangeInfo> &infos) {
    FTArray messagePinStatusInfoArray;
    for (auto &info : infos) {
        FTMap changeInfoMap;
        changeInfoMap[FTValue("pinStatus")] = FTValue(info.pinStatus);
        changeInfoMap[FTValue("message")] = cnvZIMMessageObjectToMap(info.message.get());
        messagePinStatusInfoArray.emplace_back(changeInfoMap);
    }

    return messagePinStatusInfoArray;
}
...
flutter::EncodableValue ZIMPluginConverter::cnvZIMTipsMessageChangeInfoToMap(
    ...
    if (groupChangeInfo) {
        ...
        changeInfoMap[FTValue("groupMutedInfo")] =
    ...
    if (groupMemberChangeInfo) {
        ...
        changeInfoMap[FTValue("memberRole")] =
            FTValue((int32_t)(groupMemberChangeInfo->getMemberRole()));
        if (groupMemberChangeInfo->groupNewOwner != nullptr) {
            changeInfoMap[FTValue("groupNewOwner")] =
                cnvZIMGroupMemberSimpleInfoToMap(groupMemberChangeInfo->groupNewOwner);
        }
        return changeInfoMap;
    }

    auto pinStatusChangeInfo =
        std::dynamic_pointer_cast<ZIMTipsMessagePinStatusChangeInfo>(notConstPtr);
    if (pinStatusChangeInfo) {
        changeInfoMap[FTValue("classType")] = FTValue("ZIMTipsMessagePinStatusChangeInfo");
        changeInfoMap[FTValue("isPinned")] = FTValue(pinStatusChangeInfo->isPinned());
    }
...
flutter::EncodableValue ZIMPluginConverter::cnvZIMMessageObjectToMap(ZIMMessage *message) {
    ...
    messageMap[FTValue("editorUserID")] = FTValue(message->getEditorUserID());
    messageMap[FTValue("editedTime")] = FTValue(message->getEditedTime());
    messageMap[FTValue("isGroupTargetedMessage")] = FTValue(message->isGroupTargetedMessage());
    messageMap[FTValue("pinnedUserID")] = FTValue(message->getPinnedUserID());
    messageMap[FTValue("pinnedTime")] = FTValue(message->getPinnedTime());
    ...
    case ZIM_MESSAGE_TYPE_TIPS: {
        ...
        if (tipsMessage->getChangeInfo() == nullptr) {
    ...
    case ZIM_MESSAGE_TYPE_MULTIPLE: {
        auto multipleMessage = static_cast<ZIMMultipleMessage *>(message);
        auto messageInfoArray =
            ZIMPluginConverter::cnvZIMMessageLiteInfoListToArray(multipleMessage->messageInfoList);
        auto failedIndexList =
            ZIMPluginConverter::cnvStlVectorToFTArray(multipleMessage->failedIndexList);
        messageMap[FTValue("messageInfoList")] = messageInfoArray;
        messageMap[FTValue("failedIndexList")] = failedIndexList;
        break;
    }
...
FTMap ZIMPluginConverter::cnvZIMMessageLiteInfoToMap(
    ...
    case ZIM_MESSAGE_TYPE_CUSTOM: {
        ...
        infoMap[FTValue("searchedContent")] = FTValue(customLiteInfo->searchedContent);
        break;
    }
    case ZIM_MESSAGE_TYPE_COMBINE: {
    ...
    case ZIM_MESSAGE_TYPE_MULTIPLE: {
        auto multipleLiteInfo = std::static_pointer_cast<ZIMMultipleMessageLiteInfo>(infoPtr);
        auto messageInfoArray =
            ZIMPluginConverter::cnvZIMMessageLiteInfoListToArray(multipleLiteInfo->messageInfoList);
        infoMap[FTValue("messageInfoList")] = messageInfoArray;
        break;
    }
    case ZIM_MESSAGE_TYPE_UNKNOWN:
    default:
...
FTArray ZIMPluginConverter::cnvZIMMessageLiteInfoListToArray(
    const std::vector<std::shared_ptr<ZIMMessageLiteInfo>> &infoPtrList) {
    FTArray messageInfoArray;
    for (const auto &messageInfo : infoPtrList) {
        auto messageMap = cnvZIMMessageLiteInfoToMap(messageInfo);
        if (std::holds_alternative<FTMap>(FTValue(messageMap))) {
            messageInfoArray.emplace_back(messageMap);
        }
    }
...
ZIMPluginConverter::cnvZIMConversationToObject(FTMap conversationMap) {
    ...
    conversationPtr->pinnedTime =
        (long long)ZIMPluginConverter::cnvFTValueToInt64(conversationMap[FTValue("pinnedTime")]);
    ...
    conversationPtr->readMessageSeq =
        cnvFTValueToInt64(conversationMap[FTValue("readMessageSeq")]);
    if (std::holds_alternative<std::monostate>(conversationMap[FTValue("lastMessage")])) {
...
std::shared_ptr<ZIMMessage> ZIMPluginConverter::cnvZIMMessageToObject(FTMap messageMap) {
    ...
    case zim::ZIM_MESSAGE_TYPE_MULTIPLE: {
        messagePtr = std::make_shared<ZIMMultipleMessage>();
        auto multipleMessagePtr = std::static_pointer_cast<ZIMMultipleMessage>(messagePtr);
        if (std::holds_alternative<FTArray>(messageMap[FTValue("messageInfoList")])) {
            for (auto &msgLiteInfo : std::get<FTArray>(messageMap[FTValue("messageInfoList")])) {
                if (std::holds_alternative<FTMap>(msgLiteInfo)) {
                    multipleMessagePtr->messageInfoList.emplace_back(
                        cnvZIMMessageLiteInfoToObject(std::get<FTMap>(msgLiteInfo)));
                }
            }
        }
        if (std::holds_alternative<FTArray>(messageMap[FTValue("failedIndexList")])) {
            auto failedIndexListDart = std::get<FTArray>(messageMap[FTValue("failedIndexList")]);
            multipleMessagePtr->failedIndexList =
                cnvFTArrayToStlVectorIntValue(failedIndexListDart);
        }
        break;
    }
    default:
    ...
    (*messagePtr.get()).*get(ZIM_FriendlyGet_isGroupTargetedMessage()) =
        (bool)std::get<bool>(messageMap[FTValue("isGroupTargetedMessage")]);
    ...
    (*messagePtr.get()).*get(ZIM_FriendlyGet_editorUserID()) =
        std::get<std::string>(messageMap[FTValue("editorUserID")]);
    (*messagePtr.get()).*get(ZIM_FriendlyGet_editedTime()) =
        (long long)ZIMPluginConverter::cnvFTValueToInt64(messageMap[FTValue("editedTime")]);
    (*messagePtr.get()).*get(ZIM_FriendlyGet_pinnedUserID()) =
        std::get<std::string>(messageMap[FTValue("pinnedUserID")]);
    (*messagePtr.get()).*get(ZIM_FriendlyGet_pinnedTime()) =
        (long long)ZIMPluginConverter::cnvFTValueToInt64(messageMap[FTValue("pinnedTime")]);
...
ZIMLoginConfig ZIMPluginConverter::cnvZIMLoginConfigToObject(FTMap configMap) {
    ...
    loginConfig.customStatus = std::get<std::string>(configMap[FTValue("customStatus")]);
    loginConfig.customStatusDuration = cnvFTValueToInt64(configMap[FTValue("customStatusDuration")]);
    return loginConfig;
}

std::shared_ptr<ZIMPushConfig> ZIMPluginConverter::cnvZIMPushConfigToObject(
    FTMap configMap, std::shared_ptr<ZIMVoIPConfig> &voIPConfigPtr,
    std::shared_ptr<ZIMOfflinePushPrivateMessageTemplate> &templatePtr) {
    ...
    config->threadID = std::get<std::string>(configMap[FTValue("threadID")]);
    ...
    if (std::holds_alternative<std::monostate>(configMap[FTValue("privateMessageTemplate")])) {
        templatePtr = nullptr;
        config->privateMessageTemplate = nullptr;
    } else {
        templatePtr = cnvZIMOfflinePushPrivateMessageTemplateToObject(
            std::get<FTMap>(configMap[FTValue("privateMessageTemplate")]));
        config->privateMessageTemplate = templatePtr.get();
    }
    return config;
}
...
FTMap ZIMPluginConverter::cnvZIMMessageReceiptInfoToMap(
    ...
    infoMap[FTValue("readTime")] = FTValue((int64_t)messageReceiptInfo.readTime);
    return infoMap;
}
...
FTMap ZIMPluginConverter::cnvZIMGroupMemberInfoToMap(const ZIMGroupMemberInfo &memberInfo) {
    ...
    groupMemberInfoMap[FTValue("userExtendedData")] = FTValue(memberInfo.userExtendedData);
...
FTMap ZIMPluginConverter::cnvZIMCallInfoToMap(const ZIMCallInfo &callInfo) {
    ...
    callInfoMap[FTValue("inviter")] = FTValue(callInfo.inviter);
...
FTMap ZIMPluginConverter::cnvZIMCallInvitationSentInfoToMap(const ZIMCallInvitationSentInfo &info) {
    ...
    sentInfoMap[FTValue("errorUserList")] = cnvZIMErrorUserInfoListToArray(info.errorUserList);
...
ZIMPluginConverter::cnvZIMConversationSearchConfigMapToObject(FTMap configMap) {
    ...
    config.mode =
        static_cast<ZIMConversationSearchMode>(std::get<int32_t>(configMap[FTValue("mode")]));
...
std::shared_ptr<ZIMOfflinePushPrivateMessageTemplate>
ZIMPluginConverter::cnvZIMOfflinePushPrivateMessageTemplateToObject(FTMap configMap) {
    std::shared_ptr<ZIMOfflinePushPrivateMessageTemplate> config =
        std::make_shared<ZIMOfflinePushPrivateMessageTemplate>();
    config->templateID = std::get<std::string>(configMap[FTValue("templateID")]);
    config->titleParameters = std::get<std::string>(configMap[FTValue("titleParameters")]);
    config->contentParameters = std::get<std::string>(configMap[FTValue("contentParameters")]);
    return config;
}
...
FTArray ZIMPluginConverter::cnvZIMConversationSearchInfoListToArray(
    ...
    for (auto &convInfo : conversationSearchInfoList) {
        ...
        convSearchInfoMap[FTValue("conversation")] =
            ZIMPluginConverter::cnvZIMConversationToMap(convInfo.conversation);
...
FTMap ZIMPluginConverter::cnvZIMMessageReactionToMap(const ZIMMessageReaction &reaction) {
    ...
    reactionMap[FTValue("sumCount")] = FTValue(reaction.sumCount);
    return reactionMap;
}
...
FTMap ZIMPluginConverter::mZIMMessageReactionSimpleInfo(const ZIMMessageReactionSimpleInfo &info) {
    FTMap infoMap;
    infoMap[FTValue("reactionType")] = FTValue(info.reactionType);
    infoMap[FTValue("sumCount")] = FTValue(info.sumCount);

    return infoMap;
}

FTArray ZIMPluginConverter::mZIMMessageReactionSimpleInfoList(
    const std::vector<ZIMMessageReactionSimpleInfo> &infoList) {
    FTArray simpleInfoArray;
    for (const auto &simpleInfo : infoList) {
        FTMap simpleInfoMap = ZIMPluginConverter::mZIMMessageReactionSimpleInfo(simpleInfo);
        simpleInfoArray.emplace_back(simpleInfoMap);
    }

    return simpleInfoArray;
}

FTArray ZIMPluginConverter::cnvZIMMessageReactionUserInfoListToArrayByUserFullInfoList(
    const std::vector<ZIMMessageReactionUserFullInfo> &infoList) {
    FTArray userInfoArray;
    for (const auto &userInfo : infoList) {
        FTMap userInfoMap;
        userInfoMap[FTValue("userID")] = FTValue(userInfo.userID);
        userInfoArray.emplace_back(userInfoMap);
    }

    return userInfoArray;
}

FTMap ZIMPluginConverter::mZIMMessageReactionUserFullInfo(
    const ZIMMessageReactionUserFullInfo &info) {
    FTMap infoMap;
    infoMap[FTValue("userID")] = FTValue(info.userID);
    infoMap[FTValue("reactions")] =
        ZIMPluginConverter::mZIMMessageReactionSimpleInfoList(info.reactions);

    return infoMap;
}

FTArray ZIMPluginConverter::mZIMMessageReactionUserFullInfoList(
    const std::vector<ZIMMessageReactionUserFullInfo> &infoList) {
    FTArray userInfoArray;
    for (const auto &userInfo : infoList) {
        FTMap userInfoMap = ZIMPluginConverter::mZIMMessageReactionUserFullInfo(userInfo);
        userInfoArray.emplace_back(userInfoMap);
    }
...
FTMap ZIMPluginConverter::mZIMMessageReactionUserChangeInfo(
    const ZIMMessageReactionUserChangeInfo &info) {
    FTMap infoMap;
    infoMap[FTValue("userID")] = FTValue(info.userID);
    infoMap[FTValue("reactionType")] = FTValue(info.reactionType);
    infoMap[FTValue("action")] = FTValue(info.action);
    infoMap[FTValue("sumCount")] = FTValue(info.sumCount);

    return infoMap;
}

FTArray ZIMPluginConverter::mZIMMessageReactionUserChangeInfoList(
    const std::vector<ZIMMessageReactionUserChangeInfo> &infoList) {
    FTArray changeInfoArray;
    for (const auto &changeInfo : infoList) {
        FTMap changeInfoMap = ZIMPluginConverter::mZIMMessageReactionUserChangeInfo(changeInfo);
        changeInfoArray.emplace_back(changeInfoMap);
    }

    return changeInfoArray;
}

FTMap ZIMPluginConverter::mZIMMessageReactionChangeInfo(const ZIMMessageReactionChangeInfo &info) {
    FTMap infoMap;
    infoMap[FTValue("message")] = ZIMPluginConverter::cnvZIMMessageObjectToMap(info.message.get());
    infoMap[FTValue("userChangeInfoList")] =
        ZIMPluginConverter::mZIMMessageReactionUserChangeInfoList(info.userChangeInfoList);

    return infoMap;
}

FTArray ZIMPluginConverter::mZIMMessageReactionChangeInfoList(
    const std::vector<ZIMMessageReactionChangeInfo> &infoList) {
    FTArray changeInfoArray;
    for (const auto &changeInfo : infoList) {
        FTMap changeInfoMap = ZIMPluginConverter::mZIMMessageReactionChangeInfo(changeInfo);
        changeInfoArray.emplace_back(changeInfoMap);
    }
...
ZIMMessageReactionAddConfig ZIMPluginConverter::mZIMMessageReactionAddConfig(FTMap configMap) {
    ZIMMessageReactionAddConfig config;
    config.increaseCount =
        ZIMPluginConverter::cnvFTValueToInt32(configMap[FTValue("increaseCount")]);
...
ZIMPluginConverter::cnvZIMGroupJoinApplicationSendConfigToObject(FTMap configMap) {
    ...
    if (std::holds_alternative<std::monostate>(configMap[FTValue("pushConfig")])) {
        ...
        std::shared_ptr<ZIMOfflinePushPrivateMessageTemplate> templatePtr = nullptr;
        pushConfigPtr = ZIMPluginConverter::cnvZIMPushConfigToObject(
            std::get<FTMap>(configMap[FTValue("pushConfig")]), voIPConfigPtr, templatePtr);
...
ZIMPluginConverter::oZIMUserCustomStatusUpdateConfig(FTMap configMap) {

    ZIMUserCustomStatusUpdateConfig config;
    config.duration = ZIMPluginConverter::cnvFTValueToInt64(configMap[FTValue("duration")]);
...
ZIMMessageSendConfig ZIMPluginConverter::oZIMMessageSendConfig(
    FTMap configMap, std::shared_ptr<ZIMPushConfig> &pushConfigPtr,
    ...
    config.isRetrySend = std::get<bool>(configMap[FTValue("isRetrySend")]);
    config.disableUnreadMessageCount =
        std::get<bool>(configMap[FTValue("disableUnreadMessageCount")]);
    config.isAllowMultipleMessagePartialFailed =
        std::get<bool>(configMap[FTValue("isAllowMultipleMessagePartialFailed")]);
    if (std::holds_alternative<std::monostate>(configMap[FTValue("pushConfig")])) {
        config.pushConfig = nullptr;
    } else {
        pushConfigPtr = ZIMPluginConverter::cnvZIMPushConfigToObject(
...
ZIMMediaDownloadConfig ZIMPluginConverter::oZIMMediaDownloadConfig(FTMap configMap) {
    ZIMMediaDownloadConfig config;

    config.messageInfoIndex =
        static_cast<unsigned int>(cnvFTValueToInt32(configMap[FTValue("messageInfoIndex")]));
...
ZIMPluginConverter::cnvZIMMessageLiteInfoToObject(FTMap infoMap) {
    ...
    case ZIM_MESSAGE_TYPE_CUSTOM: {
        ...
        customLiteInfo->searchedContent =
            std::get<std::string>(infoMap[FTValue("searchedContent")]);
        break;
    }
    case ZIM_MESSAGE_TYPE_COMBINE: {
    ...
    case ZIM_MESSAGE_TYPE_MULTIPLE: {
        infoPtr = std::make_shared<ZIMMultipleMessageLiteInfo>();
        auto multipleLiteInfo = std::static_pointer_cast<ZIMMultipleMessageLiteInfo>(infoPtr);
        if (std::holds_alternative<FTArray>(infoMap[FTValue("messageInfoList")])) {
            for (auto &msgLiteInfo : std::get<FTArray>(infoMap[FTValue("messageInfoList")])) {
                if (std::holds_alternative<FTMap>(msgLiteInfo)) {
                    multipleLiteInfo->messageInfoList.emplace_back(
                        cnvZIMMessageLiteInfoToObject(std::get<FTMap>(msgLiteInfo)));
                }
            }
        }
...
std::shared_ptr<ZIMUserInfo> ZIMPluginConverter::cnvZIMUserInfoToObject(FTMap infoMap) {
    ...
    info->userExtendedData = std::get<std::string>(infoMap[FTValue("userExtendedData")]);
...
FTMap ZIMPluginConverter::cnvZIMGroupMemberSimpleInfoToMap(
    ...
    infoMap[FTValue("userExtendedData")] = FTValue(operatedUser->userExtendedData);
    ...
    infoMap[FTValue("classType")] = FTValue("ZIMGroupMemberSimpleInfo");
    return infoMap;
}
...
FTMap ZIMPluginConverter::cnvZIMSelfUserInfoToMap(const ZIMSelfUserInfo &info) {
    ...
    infoMap[FTValue("userStatus")] = cnvZIMUserStatusToMap(info.userStatus);
    return infoMap;
}
...
FTMap ZIMPluginConverter::cnvZIMUserStatusToMap(const ZIMUserStatus &status) {
    ...
    statusMap[FTValue("customStatus")] = FTValue(status.customStatus);
    ...
    statusMap[FTValue("lastOnlineStatusUpdateTime")] = (int64_t)status.lastOnlineStatusUpdateTime;
    statusMap[FTValue("lastCustomStatusUpdateTime")] = (int64_t)status.lastCustomStatusUpdateTime;
    statusMap[FTValue("customStatusExpiredTime")] = (int64_t)status.customStatusExpiredTime;
    return statusMap;
}
...
FTMap ZIMPluginConverter::cnvZIMFriendInfoToMap(const ZIMFriendInfo &info) {
    ...
    infoMap[FTValue("userExtendedData")] = FTValue(info.userExtendedData);
...
FTMap ZIMPluginConverter::mZIMMessageReactionsChangedEventResult(
    const ZIMMessageReactionsChangedEventResult &result) {
    FTMap resultMap;

    resultMap[FTValue("reactions")] =
        ZIMPluginConverter::cnvZIMMessageReactionListToArray(result.reactions);
    resultMap[FTValue("changeInfoList")] =
        ZIMPluginConverter::mZIMMessageReactionChangeInfoList(result.changeInfoList);
