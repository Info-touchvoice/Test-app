...
#include "ZegoLog.h"
#ifdef _WIN32
#include "zego_express_engine/ZegoAudioDataManager.h"
#include "zego_express_engine/ZegoCustomAudioProcessManager.h"
#include "zego_express_engine/ZegoCustomEventManager.h"
#endif
...
void ZegoExpressEngineEventHandler::onRoomStateChanged(const std::string &roomID,
    ...
    if (eventSink_) {
        ...
        if (EXPRESS::ZegoRoomStateChangedReason::ZEGO_ROOM_STATE_CHANGED_REASON_LOGINED == reason && !extendedData.empty())
        {
            extendedData_ = ZF::getApiTimeExtendedJsonData(extendedData_);
        }
...
void ZegoExpressEngineEventHandler::onRoomStreamUpdate(
    ...
    ZF::logInfo(
        "[onRoomStreamUpdate] roomID: %s, updateType: %d, streamListCount: %d, extendedData :%s",
        roomID.c_str(), updateType, streamList.size(), extendedData.c_str());

    std::string extended_data = extendedData;
    if (!extendedData.empty() && EXPRESS::ZegoUpdateType::ZEGO_UPDATE_TYPE_ADD == updateType)
    {
        extended_data = ZF::getApiTimeExtendedJsonData(extendedData, true);
    }
    ...
    if (eventSink_) {
        ...
        retMap[ZFValue("updateType")] = ZFValue(updateType);
        retMap[ZFValue("roomID")] = ZFValue(roomID);
        retMap[ZFValue("extendedData")] = ZFValue(extended_data);
...
void ZegoExpressEngineEventHandler::onPublisherStateUpdate(const std::string &streamID,
    ...
    std::string extended_data = extendedData;
    if (!extendedData.empty() && state == EXPRESS::ZegoPublisherState::ZEGO_PUBLISHER_STATE_PUBLISHING) {
        extended_data = ZF::getApiTimeExtendedJsonData(extendedData);
    }
...
ZFMap ZegoExpressEngineEventHandler::convertPublishQuality(
    ...
    qualityMap[ZFValue("audioTrafficControlRate")] = ZFValue(quality.audioTrafficControlRate);
    qualityMap[ZFValue("videoTrafficControlRate")] = ZFValue(quality.videoTrafficControlRate);
...
// windows not supported onPublisherFaceDetectInfo
...
void ZegoExpressEngineEventHandler::onPlayerStateUpdate(const std::string &streamID,
    ...
    std::string extended_data = extendedData;
    if (!extendedData.empty() && EXPRESS::ZegoPlayerState::ZEGO_PLAYER_STATE_PLAYING == state) {
        extended_data = ZF::getApiTimeExtendedJsonData(extendedData);
    }
...
#ifdef _WIN32
    auto customEventHandler = ZegoCustomEventManager::getInstance()->getHandler();
    if(customEventHandler) {
        customEventHandler->onPlayerRecvMediaSideInfo(dataArray.data(), dataArray.size(), info.streamID.c_str(), info.timestampNs);
    }
#endif
...
#ifdef _WIN32
    auto appHandler = ZegoAudioDataManager::getInstance()->getHandler();
    if (appHandler) {
        appHandler->onCapturedAudioData(data, dataLength, param);
    }
#endif
...
void ZegoExpressEngineEventHandler::onPlaybackAudioData(const unsigned char *data,
    ...
    auto appHandler = ZegoAudioDataManager::getInstance()->getHandler();
    if (appHandler) {
        appHandler->onPlaybackAudioData(data, dataLength, param);
    }
#endif
...
void ZegoExpressEngineEventHandler::onMixedAudioData(const unsigned char *data,
    ...
    auto appHandler = ZegoAudioDataManager::getInstance()->getHandler();
    if (appHandler) {
        appHandler->onMixedAudioData(data, dataLength, param);
    }
#endif
...
void ZegoExpressEngineEventHandler::onPlayerAudioData(const unsigned char *data,
    ...
    auto appHandler = ZegoAudioDataManager::getInstance()->getHandler();
    if (appHandler) {
        appHandler->onPlayerAudioData(data, dataLength, param, streamID);
    }
#endif
...
void ZegoExpressEngineEventHandler::onRemoteDataRecordStateUpdate(
    EXPRESS::ZegoDataRecordState state, int errorCode, EXPRESS::ZegoDataRecordConfig config, const std::string & streamID) {

    ZF::logInfo("[onRemoteDataRecordStateUpdate] state: %d, errorCode: %d, filePath: %s, "
                "recordType: %d, streamID: %s",
                state, errorCode, config.filePath.c_str(), config.recordType, streamID.c_str());

    if (eventSink_) {
        ZFMap retMap;
        ZFMap configMap;
        retMap[ZFValue("method")] = ZFValue("onRemoteDataRecordStateUpdate");
...
void ZegoExpressEngineEventHandler::onRemoteDataRecordProgressUpdate(
    EXPRESS::ZegoDataRemoteRecordProgress progress, EXPRESS::ZegoDataRecordConfig config, const std::string & streamID) {
    ...
    if (eventSink_) {
        ...
        ZFMap progressMap;
        ZFMap configMap;
        retMap[ZFValue("method")] = ZFValue("onRemoteDataRecordProgressUpdate");

        progressMap[ZFValue("currentFileSize")] = ZFValue((int32_t)progress.currentFileSize);
        progressMap[ZFValue("duration")] = ZFValue((int32_t)progress.duration);

        auto & quality = progress.quality;
...
void ZegoExpressEngineEventHandler::onRtcStats(const EXPRESS::ZegoRtcStatsInfo &info) {

    // High frequency callbacks do not log

    if (eventSink_) {
        ZFMap retMap;
        retMap[ZFValue("method")] = ZFValue("onRtcStats");
        ZFMap infoMap;
        infoMap[ZFValue("totalTxBandwidth")] = ZFValue(info.totalTxBandwidth);
        infoMap[ZFValue("avgTxRtt")] = ZFValue((int64_t)info.avgTxRtt);
        infoMap[ZFValue("avgTxPacketLostRate")] = ZFValue(info.avgTxPacketLostRate);
        infoMap[ZFValue("totalRxBandwidth")] = ZFValue(info.totalRxBandwidth);
        infoMap[ZFValue("avgRxRtt")] = ZFValue((int64_t)info.avgRxRtt);
        infoMap[ZFValue("avgRxPacketLostRate")] = ZFValue(info.avgRxPacketLostRate);
        infoMap[ZFValue("avgPeerToPeerDelay")] = ZFValue((int64_t)info.avgPeerToPeerDelay);

        retMap[ZFValue("info")] = ZFValue(infoMap);
...
#ifdef _WIN32
    if (ZegoCustomAudioProcessManager::getInstance()->getHandler()) {
        ...
        ZegoCustomAudioProcessManager::getInstance()->getHandler()->onProcessCapturedAudioData(data, dataLength, &frameParam, timestamp);
        param->sampleRate = (ZEGO::EXPRESS::ZegoAudioSampleRate)frameParam.sampleRate;
        param->channel = (ZEGO::EXPRESS::ZegoAudioChannel)frameParam.channel;
    }
#endif
}
...
void ZegoExpressEngineEventHandler::onProcessCapturedAudioDataAfterUsedHeadphoneMonitor(
    ...
    if (ZegoCustomAudioProcessManager::getInstance()->getHandler()) {
        ...
        frameParam.sampleRate = (ZGFlutterAudioSampleRate)param->sampleRate;
        frameParam.channel = (ZGFlutterAudioChannel)param->channel;
        ZegoCustomAudioProcessManager::getInstance()->getHandler()->onProcessCapturedAudioDataAfterUsedHeadphoneMonitor(data, dataLength, &frameParam, timestamp);
        param->sampleRate = (ZEGO::EXPRESS::ZegoAudioSampleRate)frameParam.sampleRate;
        param->channel = (ZEGO::EXPRESS::ZegoAudioChannel)frameParam.channel;
    }
#endif
...
void ZegoExpressEngineEventHandler::onBeforeAudioPrepAudioData(const unsigned char *data,
                                                                 unsigned int dataLength,
                                                                 EXPRESS::ZegoAudioFrameParam param) {
    // High frequency callbacks do not log
    ...
    if (eventSink_) {
        ZFMap retMap;
        retMap[ZFValue("method")] = ZFValue("onBeforeAudioPrepAudioData");
        std::vector<uint8_t> dataArray(data, data + dataLength);
        retMap[ZFValue("data")] = ZFValue(dataArray);
        retMap[ZFValue("dataLength")] = ZFValue((int64_t)dataLength);
        ZFMap paramMap;
    ...
    if (ZegoCustomAudioProcessManager::getInstance()->getHandler()) {
        ...
        frameParam.sampleRate = (ZGFlutterAudioSampleRate)param.sampleRate;
        frameParam.channel = (ZGFlutterAudioChannel)param.channel;
        ZegoCustomAudioProcessManager::getInstance()->getHandler()->onBeforeAudioPrepAudioData((unsigned char *)data, dataLength, &frameParam);
        param.sampleRate = (ZEGO::EXPRESS::ZegoAudioSampleRate)frameParam.sampleRate;
        param.channel = (ZEGO::EXPRESS::ZegoAudioChannel)frameParam.channel;
    }
#endif
...
void ZegoExpressEngineEventHandler::onProcessRemoteAudioData(unsigned char *data,
    ...
    if (ZegoCustomAudioProcessManager::getInstance()->getHandler()) {
        ...
        frameParam.sampleRate = (ZGFlutterAudioSampleRate)param->sampleRate;
        frameParam.channel = (ZGFlutterAudioChannel)param->channel;
        ZegoCustomAudioProcessManager::getInstance()->getHandler()->onProcessRemoteAudioData(data, dataLength, &frameParam, streamID, timestamp);
        param->sampleRate = (ZEGO::EXPRESS::ZegoAudioSampleRate)frameParam.sampleRate;
        param->channel = (ZEGO::EXPRESS::ZegoAudioChannel)frameParam.channel;
    }
#endif
...
void ZegoExpressEngineEventHandler::onProcessPlaybackAudioData(unsigned char *data,
    ...
    if (ZegoCustomAudioProcessManager::getInstance()->getHandler()) {
        ...
        frameParam.sampleRate = (ZGFlutterAudioSampleRate)param->sampleRate;
        frameParam.channel = (ZGFlutterAudioChannel)param->channel;
        ZegoCustomAudioProcessManager::getInstance()->getHandler()->onProcessPlaybackAudioData(data, dataLength, &frameParam, timestamp);
        param->sampleRate = (ZEGO::EXPRESS::ZegoAudioSampleRate)frameParam.sampleRate;
        param->channel = (ZEGO::EXPRESS::ZegoAudioChannel)frameParam.channel;
    }
#endif
...
void ZegoExpressEngineEventHandler::onExceptionOccurred(
    ...
    if (!ZegoExpressEngineMethodHandler::getInstance().isScreenCaptureSourcesValid()) {
        return;
    }
