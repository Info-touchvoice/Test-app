...
class ZegoExpressTextureRenderer {
  ...
  Future<int> createTextureRenderer(int width, int height,
    ...
    final int textureID = await ZegoExpressImpl.methodChannel.invokeMethod(
        ...
        {'width': width, 'height': height, 'args': args});
  ...
  Future<bool> destroyTextureRenderer(int textureID) async {
    ZegoExpressTextureRenderer().removeTexture(textureID);
  ...
  void setRotation(int textureID, int rotation) {
    if (!kIsWindows) {
      return;
    }
    ZegoExpressImpl.sendCustomLogMessage(
        '[WidgetState] setRotation. widget: $textureID, rotation: $rotation');
    _rotationMap[textureID] = rotation;
    _updateController.sink.add({'textureID': textureID, 'type': 'update'});
  }

  void setMirror(int textureID, bool mirror) {
    if (!kIsWindows) {
      return;
    }
    ZegoExpressImpl.sendCustomLogMessage(
        '[WidgetState] setMirror. widget: $textureID, mirror: $mirror');
    _mirrorMap[textureID] = mirror ? 1 : 0;
    _updateController.sink.add({'textureID': textureID, 'type': 'update'});
  }

  void updateRendererSize(int textureID, int width, int height) {
    ZegoExpressImpl.sendCustomLogMessage(
        '[WidgetState] updateRendererSize. textureID: $textureID, width: $width, height: $height');
    _sizeMap[textureID] =
        Size(double.parse(width.toString()), double.parse(height.toString()));
    _updateController.sink.add({'textureID': textureID, 'type': 'update'});
  }
  ...
  bool removeTexture(int textureID) {
    ...
    _backgroundColorMap.remove(textureID);
    _sizeMap.remove(textureID);
    _mirrorMap.remove(textureID);
    _rotationMap.remove(textureID);
    clearWidgetStreamSubscription('$textureID#');
    return true;
  }
