...
class ZegoExpressEngineWeb {
  ...
  Future<dynamic> handleMethodCall(MethodCall call) async {
    ...
    switch (call.method) {
      ...
      case 'setPlayStreamVideoType':
        return setPlayStreamVideoType(
            call.arguments['streamID'], call.arguments['streamType']);
  ...
  static void _eventListener(dynamic event) {
    ...
    switch (methodName) {
      ...
      case 'onReceiveRealTimeSequentialData':
        ...
        // print("onReceiveRealTimeSequentialData ${map['data']}");
        final data = jsonDecode(map["data"]);
      ...
      case 'onMixerRelayCDNStateUpdate':
        final data = jsonDecode(map["data"]);
        List<dynamic> infoMapList = data['infoList'];
        // print('onMixerRelayCDNStateUpdate $infoMapList');
        ...
        ZegoExpressEngine.onMixerRelayCDNStateUpdate!(data['taskID'], infoList);
        break;
      //////////////////////////////媒体播放器//////////////////////////
  ...
  static int getPublishChannel([int? channel = 0]) {
    return channel ?? 0;
  }
  ...
  Future<void> setPlayStreamVideoType(String streamID, int streamType) async {
  ...
  Future<Map<dynamic, dynamic>> sendRealTimeSequentialData(
    ...
    final Map<dynamic, dynamic> map = {};
    // print("sendRealTimeSequentialData ${data}");
    await ZegoFlutterEngine.instance
        .sendRealTimeSequentialData(index, streamID, base64Encode(data));
    map["errorCode"] = 0;
    return Future.value(map);
  }
  ...
  Future<int> createScreenCaptureSource(int? sourceId, int? sourceType) async {
    var instance = ZegoFlutterEngine.instance.createScreenCaptureSource(_index);
    var i = _index;
    // var capureSource = ScreenCaptureSource();
    // capureSource.instance = instance;
    _mediaSources[i] = instance;
    _index++;
    return i;
  }
  ...
  Future<void> startCaptureScreen(int index, dynamic config) async {
    final promise =
        callMethod(_mediaSources[index].instance, 'startCapture', [config]);
    final result = await promiseToFuture(promise);
    return result;
  }

  Future<void> stopCaptureScreen(int index) async {
    return callMethod(_mediaSources[index].instance, 'stopCapture', []);
  }
