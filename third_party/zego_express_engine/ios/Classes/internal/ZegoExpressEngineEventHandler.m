...
NSString* getApicostTime(NSDictionary *extendedData, BOOL is_serverpush)
 {
    NSMutableDictionary *modifiedData = extendedData ? [extendedData mutableCopy] : [NSMutableDictionary dictionary];
    ...
    if (is_serverpush) 
    {
        modifiedData[@"push_time"] = @(currentTime);
    }
    else 
    {
        if (extendedData[@"call_api_time"])
         {
            long long callApiTime = [extendedData[@"call_api_time"] longLongValue];
            long long costTime = currentTime - callApiTime;
            modifiedData[@"callback_time"] = @(currentTime);
            modifiedData[@"cost_time"] = @(costTime);
        } 
    }

    NSError *error = nil;
    NSData *jsonData = [NSJSONSerialization dataWithJSONObject:modifiedData  options:0 error:&error];
...
- (void)onRoomStateChanged:(ZegoRoomStateChangedReason)reason errorCode:(int)errorCode extendedData:(NSDictionary *)extendedData roomID:(NSString *)roomID {
    ...
    if (sink) {
        NSString *extendedDataJsonString = @"{}";
        if (extendedData) {
            if(ZegoRoomStateChangedReasonLogined == reason)
            {
                extendedDataJsonString = getApicostTime(extendedData, false);
            }
            else
            {
                NSError *error;
...
- (void)onRoomStreamUpdate:(ZegoUpdateType)updateType streamList:(NSArray<ZegoStream *> *)streamList extendedData:(NSDictionary *)extendedData roomID:(NSString *)roomID {
    ...
    if (sink) {
        ...
        NSString *extendedDataJsonString = @"{}";
        if (extendedData) {
            if(ZegoUpdateTypeAdd == updateType)
            {
                 extendedDataJsonString = getApicostTime(extendedData, true);
            }
            else
            {
                NSError *error;
...
- (void)onPublisherStateUpdate:(ZegoPublisherState)state errorCode:(int)errorCode extendedData:(NSDictionary *)extendedData streamID:(NSString *)streamID {
    ...
    if (sink) {
        NSString *extendedDataJsonString = @"{}";
        if (extendedData) {
            if(ZegoPublisherStatePublishing == state && 0 == errorCode)
            {
                extendedDataJsonString = getApicostTime(extendedData, false);
            }
            else
...
- (NSDictionary *)convertPublishQuality:(ZegoPublishStreamQuality *)quality {
    return @{
                ...
                @"audioTrafficControlRate": @(quality.audioTrafficControlRate),
                @"videoTrafficControlRate": @(quality.videoTrafficControlRate)
            };
}
...
- (void)onPublisherFaceDetectInfo:(ZegoFaceDetectionInfo *)info channel:(ZegoPublishChannel)channel {
    FlutterEventSink sink = _eventSink;
    ZGLog(@"[onPublisherFaceDetectInfo] size: %d, w: %d, h: %d, channel: %d", info.facePositionList.count, info.imageWidth, info.imageHeight, (int)channel);

    GUARD_SINK
    if (sink) {
        NSMutableArray *facePositionListArray = [[NSMutableArray alloc] init];
        for (ZegoFacePositionInfo *rect in info.facePositionList) {
            [facePositionListArray addObject:@{
                @"left": @(rect.position.origin.x),
                @"top": @(rect.position.origin.y),
                @"right": @(CGRectGetMaxX(rect.position)),
                @"bottom": @(CGRectGetMaxY(rect.position))
            }];
        }
        sink(@{
            @"method": @"onPublisherFaceDetectInfo",
            @"info": @{
                @"errorCode": @(info.errorCode),
                @"imageWidth": @(info.imageWidth),
                @"imageHeight": @(info.imageHeight),
                @"facePositionList": facePositionListArray
            },
            @"channel": @((int)channel),
        });
    }
...
- (void)onPlayerStateUpdate:(ZegoPlayerState)state errorCode:(int)errorCode extendedData:(NSDictionary *)extendedData streamID:(NSString *)streamID {
    ...
    if (sink) {
        ...
        if (extendedData) {
            ...
            if(ZegoPlayerStatePlaying == state && 0 == errorCode)
            {
                extendedDataJsonString = getApicostTime(extendedData, false);
            }
            else
...
- (void)onPlayerRecvMediaSideInfo:(ZegoMediaSideInfo *)info {
    FlutterEventSink sink = _eventSink;
    // Do not log high frequency callback
    id<ZegoFlutterCustomEventHandler> customEventHandler = [[ZegoCustomEventManager sharedInstance] getCustomEventHandler];

    if(customEventHandler && [customEventHandler respondsToSelector:@selector(onPlayerRecvMediaSideInfo:streamID:timestampNs:)]) {
        [customEventHandler onPlayerRecvMediaSideInfo:info.SEIData streamID:info.streamID timestampNs:info.timestampNs];
    }
...
- (void)onPlayerRenderMultiViewFirstFrame:(NSString *) streamID viewContext:(NSString *)viewContext {
    FlutterEventSink sink = _eventSink;
    ZGLog(@"[onPlayerRenderMultiViewFirstFrame] streamID: %@ ctx:%@", streamID, viewContext);

    GUARD_SINK
    
    if (sink) {
        sink(@{
            @"method": @"onPlayerRenderMultiViewFirstFrame",
            @"streamID": streamID,
...
- (void)onRtcStats:(ZegoRtcStatsInfo *)info {
    ...
    if (sink) {
        sink(@{
            @"method": @"onRtcStats",
            @"info": @{
                @"totalTxBandwidth": @(info.totalTxBandwidth),
                @"avgTxRtt": @(info.avgTxRtt),
                @"avgTxPacketLostRate": @(info.avgTxPacketLostRate),
                @"totalRxBandwidth": @(info.totalRxBandwidth),
                @"avgRxRtt": @(info.avgRxRtt),
                @"avgRxPacketLostRate": @(info.avgRxPacketLostRate),
                @"avgPeerToPeerDelay": @(info.avgPeerToPeerDelay)
            },
        });
    }
}
...
- (void)onMobileScreenCaptureStart {
    FlutterEventSink sink = _eventSink;
    ZGLog(@"[onMobileScreenCaptureStart]");

    GUARD_SINK
    if (sink) {
        sink(@{
            @"method": @"onMobileScreenCaptureStart",
        });
    }
}
...
- (void)onMobileScreenCaptureExceptionOccurred:(ZegoScreenCaptureExceptionType)exceptionType {
    FlutterEventSink sink = _eventSink;
    ZGLog(@"[onMobileScreenCaptureExceptionOccurred] exceptionType: %d", (int)exceptionType);

    GUARD_SINK
    if (sink) {
        sink(@{
            @"method": @"onMobileScreenCaptureExceptionOccurred",
            @"exceptionType": @(exceptionType)
        });
    }
}
...
- (void)onCapturedAudioData:(const unsigned char * _Nonnull)data dataLength:(unsigned int)dataLength param:(ZegoAudioFrameParam *)param {
    ...
    id<ZegoFlutterAudioDataHandler> handler = [ZegoAudioDataManager.sharedInstance getHandler];
    if ([handler respondsToSelector:@selector(onCapturedAudioData:dataLength:param:)]) {
        ...
        [handler onCapturedAudioData:data dataLength:dataLength param:audioFrameParam];
    }
}
...
- (void)onPlaybackAudioData:(const unsigned char * _Nonnull)data dataLength:(unsigned int)dataLength param:(ZegoAudioFrameParam *)param {
    ...
    id<ZegoFlutterAudioDataHandler> handler = [ZegoAudioDataManager.sharedInstance getHandler];
    if ([handler respondsToSelector:@selector(onPlaybackAudioData:dataLength:param:)]) {
        ZGFlutterAudioFrameParam *audioFrameParam = [[ZGFlutterAudioFrameParam alloc] init];
        audioFrameParam.sampleRate = (ZGFlutterAudioSampleRate)param.sampleRate;
        audioFrameParam.channel = (ZGFlutterAudioChannel)param.channel;
        [handler onPlaybackAudioData:data dataLength:dataLength param:audioFrameParam];
    }
}
...
- (void)onMixedAudioData:(const unsigned char * _Nonnull)data dataLength:(unsigned int)dataLength param:(ZegoAudioFrameParam *)param {
    ...
    id<ZegoFlutterAudioDataHandler> handler = [ZegoAudioDataManager.sharedInstance getHandler];
    if ([handler respondsToSelector:@selector(onMixedAudioData:dataLength:param:)]) {
        ZGFlutterAudioFrameParam *audioFrameParam = [[ZGFlutterAudioFrameParam alloc] init];
        audioFrameParam.sampleRate = (ZGFlutterAudioSampleRate)param.sampleRate;
        audioFrameParam.channel = (ZGFlutterAudioChannel)param.channel;
        [handler onMixedAudioData:data dataLength:dataLength param:audioFrameParam];
    }
}
...
- (void)onPlayerAudioData:(const unsigned char * _Nonnull)data dataLength:(unsigned int)dataLength param:(ZegoAudioFrameParam *)param streamID:(NSString *)streamID {
    ...
    id<ZegoFlutterAudioDataHandler> handler = [ZegoAudioDataManager.sharedInstance getHandler];
    if ([handler respondsToSelector:@selector(onPlayerAudioData:dataLength:param:streamID:)]) {
        ZGFlutterAudioFrameParam *audioFrameParam = [[ZGFlutterAudioFrameParam alloc] init];
        audioFrameParam.sampleRate = (ZGFlutterAudioSampleRate)param.sampleRate;
        audioFrameParam.channel = (ZGFlutterAudioChannel)param.channel;
        [handler onPlayerAudioData:data dataLength:dataLength param:audioFrameParam streamID:streamID];
    }
}
...
- (void)onProcessCapturedAudioData:(unsigned char *)data dataLength:(unsigned int)dataLength param:(ZegoAudioFrameParam *)param timestamp:(double)timestamp {
    ...
    if ([ZegoCustomAudioProcessManager.sharedInstance.getHandler respondsToSelector:@selector(onProcessCapturedAudioData:dataLength:param:timestamp:)]) {
        ZGFlutterAudioFrameParam *audioFrameParam = [[ZGFlutterAudioFrameParam alloc] init];
        audioFrameParam.sampleRate = (ZGFlutterAudioSampleRate)param.sampleRate;
        audioFrameParam.channel = (ZGFlutterAudioChannel)param.channel;
        [ZegoCustomAudioProcessManager.sharedInstance.getHandler onProcessCapturedAudioData:data dataLength:dataLength param:audioFrameParam timestamp:timestamp];
        param.sampleRate = (ZegoAudioSampleRate)audioFrameParam.sampleRate;
        param.channel = (ZegoAudioChannel)audioFrameParam.channel;
    }
}
...
- (void)onProcessCapturedAudioDataAfterUsedHeadphoneMonitor:(unsigned char *)data dataLength:(unsigned int)dataLength param:(ZegoAudioFrameParam *)param timestamp:(double)timestamp {
    ...
    if ([ZegoCustomAudioProcessManager.sharedInstance.getHandler respondsToSelector:@selector(onProcessCapturedAudioDataAfterUsedHeadphoneMonitor:dataLength:param:timestamp:)]) {
        ZGFlutterAudioFrameParam *audioFrameParam = [[ZGFlutterAudioFrameParam alloc] init];
        audioFrameParam.sampleRate = (ZGFlutterAudioSampleRate)param.sampleRate;
        audioFrameParam.channel = (ZGFlutterAudioChannel)param.channel;
        [ZegoCustomAudioProcessManager.sharedInstance.getHandler onProcessCapturedAudioDataAfterUsedHeadphoneMonitor:data dataLength:dataLength param:audioFrameParam timestamp:timestamp];
        param.sampleRate = (ZegoAudioSampleRate)audioFrameParam.sampleRate;
        param.channel = (ZegoAudioChannel)audioFrameParam.channel;
    }
}
...
- (void)onBeforeAudioPrepAudioData:(const unsigned char * _Nonnull)data dataLength:(unsigned int)dataLength param:(ZegoAudioFrameParam *)param {
    ...
    if (sink) {
        sink(@{
            @"method": @"onBeforeAudioPrepAudioData",
            @"data": objData,
            @"dataLength": @(dataLength),
            @"param": @{
                @"sampleRate": @(param.sampleRate),
    ...
    if ([ZegoCustomAudioProcessManager.sharedInstance.getHandler respondsToSelector:@selector(onBeforeAudioPrepAudioData:dataLength:param:)]) {
        ZGFlutterAudioFrameParam *audioFrameParam = [[ZGFlutterAudioFrameParam alloc] init];
        audioFrameParam.sampleRate = (ZGFlutterAudioSampleRate)param.sampleRate;
        audioFrameParam.channel = (ZGFlutterAudioChannel)param.channel;
        [ZegoCustomAudioProcessManager.sharedInstance.getHandler onBeforeAudioPrepAudioData:data dataLength:dataLength param:audioFrameParam];
        param.sampleRate = (ZegoAudioSampleRate)audioFrameParam.sampleRate;
        param.channel = (ZegoAudioChannel)audioFrameParam.channel;
    }
}
...
- (void)onAlignedAudioAuxData:(const unsigned char * _Nonnull) data dataLength:(unsigned int) dataLength param:(ZegoAudioFrameParam *) param {
    ...
    if ([ZegoCustomAudioProcessManager.sharedInstance.getHandler respondsToSelector:@selector(onAlignedAudioAuxData:dataLength:param:)]) {
        ZGFlutterAudioFrameParam *audioFrameParam = [[ZGFlutterAudioFrameParam alloc] init];
        audioFrameParam.sampleRate = (ZGFlutterAudioSampleRate)param.sampleRate;
        audioFrameParam.channel = (ZGFlutterAudioChannel)param.channel;
        [ZegoCustomAudioProcessManager.sharedInstance.getHandler onAlignedAudioAuxData:data dataLength:dataLength param:audioFrameParam];
        param.sampleRate = (ZegoAudioSampleRate)audioFrameParam.sampleRate;
        param.channel = (ZegoAudioChannel)audioFrameParam.channel;
    }
}
...
- (void)onProcessRemoteAudioData:(unsigned char *)data dataLength:(unsigned int)dataLength param:(ZegoAudioFrameParam *)param streamID:(NSString *)streamID timestamp:(double)timestamp {
    ...
    if ([ZegoCustomAudioProcessManager.sharedInstance.getHandler respondsToSelector:@selector(onProcessRemoteAudioData:dataLength:param:streamID:timestamp:)]) {
        ZGFlutterAudioFrameParam *audioFrameParam = [[ZGFlutterAudioFrameParam alloc] init];
        audioFrameParam.sampleRate = (ZGFlutterAudioSampleRate)param.sampleRate;
        audioFrameParam.channel = (ZGFlutterAudioChannel)param.channel;
        [ZegoCustomAudioProcessManager.sharedInstance.getHandler onProcessRemoteAudioData:data dataLength:dataLength param:audioFrameParam streamID:streamID timestamp:timestamp];
        param.sampleRate = (ZegoAudioSampleRate)audioFrameParam.sampleRate;
        param.channel = (ZegoAudioChannel)audioFrameParam.channel;
    }
}
...
- (void)onProcessPlaybackAudioData:(unsigned char *)data dataLength:(unsigned int)dataLength param:(ZegoAudioFrameParam *)param timestamp:(double)timestamp {
    ...
    if ([ZegoCustomAudioProcessManager.sharedInstance.getHandler respondsToSelector:@selector(onProcessPlaybackAudioData:dataLength:param:timestamp:)]) {
        ZGFlutterAudioFrameParam *audioFrameParam = [[ZGFlutterAudioFrameParam alloc] init];
        audioFrameParam.sampleRate = (ZGFlutterAudioSampleRate)param.sampleRate;
        audioFrameParam.channel = (ZGFlutterAudioChannel)param.channel;
        [ZegoCustomAudioProcessManager.sharedInstance.getHandler onProcessPlaybackAudioData:data dataLength:dataLength param:audioFrameParam timestamp:timestamp];
        param.sampleRate = (ZegoAudioSampleRate)audioFrameParam.sampleRate;
        param.channel = (ZegoAudioChannel)audioFrameParam.channel;
    }
}
