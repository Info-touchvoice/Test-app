...
import im.zego.zego_express_engine.ZGFlutterAudioChannel;
import im.zego.zego_express_engine.ZGFlutterAudioFrameParam;
import im.zego.zego_express_engine.ZGFlutterAudioSampleRate;
import im.zego.zego_express_engine.ZegoAudioDataManager;
import im.zego.zego_express_engine.ZegoCustomAudioProcessManager;
import im.zego.zego_express_engine.IZegoFlutterCustomEventHandler;
import im.zego.zego_express_engine.ZegoCustomEventManager;
...
public class ZegoExpressEngineEventHandler {
    ...
    public static String getApiCostTime(JSONObject extendedData, boolean isServerPush) {
        JSONObject modifiedData;

        try {
            // 复制原数据或创建新对象
            modifiedData = (extendedData != null) ? new JSONObject(extendedData.toString()) : new JSONObject();
        } catch (JSONException e) {
            e.printStackTrace();
            modifiedData = new JSONObject(); // 回退为一个空对象
        }
        ...
        try {
            if (isServerPush) {
                modifiedData.put("push_time", currentTime);
            } else {
                if (modifiedData.has("call_api_time")) {
                    long callApiTime = modifiedData.optLong("call_api_time", 0);
                    long costTime = currentTime - callApiTime;
                    modifiedData.put("callback_time", currentTime);
                    modifiedData.put("cost_time", costTime);
                }
            }
        } catch (JSONException e) {
            e.printStackTrace();
    ...
    private HashMap<String, Object> convertPublishStreamQuality(ZegoPublishStreamQuality quality) {
        ...
        qualityMap.put("audioTrafficControlRate", quality.audioTrafficControlRate);
        qualityMap.put("videoTrafficControlRate", quality.videoTrafficControlRate);
    ...
    IZegoEventHandler eventHandler = new IZegoEventHandler() {
        ...
        public void onRoomStateChanged(String roomID, ZegoRoomStateChangedReason reason, int errorCode, JSONObject extendedData) {
            ...
            HashMap<String, Object> map = new HashMap<>();
            String extended_data = extendedData.toString();

            if(ZegoRoomStateChangedReason.LOGINED == reason && errorCode == 0)
            {
                extended_data = getApiCostTime(extendedData, false);
            }
            ...
            map.put("errorCode", errorCode);
            map.put("extendedData", extended_data);
        ...
        public void onRoomStreamUpdate(String roomID, ZegoUpdateType updateType, ArrayList<ZegoStream> streamList, JSONObject extendedData) {
            ...
            {
                extended_data = getApiCostTime(extendedData, true);
            }
        ...
        public void onPublisherStateUpdate(String streamID, ZegoPublisherState state, int errorCode, JSONObject extendedData) {
            ...
            if(ZegoPublisherState.PUBLISHING == state && errorCode == 0)
            {
                extended_data = getApiCostTime(extendedData, false);
            }
        ...
        public void onPublisherVideoSizeChanged(int width, int height, ZegoPublishChannel channel) {
            ...
            map.put("channel", channel.value());
            // For Android Texture rendering with ZegoSurfaceTextureRenderer, attach textureID if available
            Long textureID = ZegoTextureRendererController.getInstance().previewTextureInUse.get(channel);
            if (textureID != null) {
                map.put("textureID", textureID.intValue());
            }
        ...
        public void onPublisherFaceDetectInfo(ZegoFaceDetectionInfo info, ZegoPublishChannel channel) {
            ...
            infoMap.put("errorCode", info.errorCode);
        ...
        public void onPlayerStateUpdate(String streamID, ZegoPlayerState state, int errorCode, JSONObject extendedData) {
            ...
            if(ZegoPlayerState.PLAYING == state && errorCode == 0)
            {
                extended_data = getApiCostTime(extendedData, false);
            }
        ...
        public void onPlayerVideoSizeChanged(String streamID, int width, int height) {
            ...
            map.put("height", height);
            // For Android Texture rendering with ZegoSurfaceTextureRenderer, attach textureID if available
            Long textureID = ZegoTextureRendererController.getInstance().playerTextureInUse.get(streamID);
            if (textureID != null) {
                map.put("textureID", textureID.intValue());
            }
        ...
        public void onPlayerRecvMediaSideInfo(ZegoMediaSideInfo info) {
            ...
            IZegoFlutterCustomEventHandler customEventHandler = ZegoCustomEventManager.getInstance().getCustomEventHandler();
            if (customEventHandler != null) {
                customEventHandler.onPlayerRecvMediaSideInfo(ByteBuffer.wrap(data), info.streamID, info.timestampNs);
            }
        ...
        @Override
        public void onPlayerRenderMultiViewFirstFrame(String streamID, String viewContext) {
            super.onPlayerRenderMultiViewFirstFrame(streamID, viewContext);
            ZegoLog.log("[onPlayerRenderMultiViewFirstFrame] streamID: %s ctx: %s", streamID, viewContext);

            if (guardSink()) { return; }

            HashMap<String, Object> map = new HashMap<>();
            map.put("method", "onPlayerRenderMultiViewFirstFrame");
            map.put("streamID", streamID);
            map.put("viewContext", viewContext);
    ...
    IZegoMediaPlayerEventHandler mediaPlayerEventHandler = new IZegoMediaPlayerEventHandler() {
        ...
        public void onMediaPlayerVideoSizeChanged(ZegoMediaPlayer mediaPlayer, int width, int height) {
            ...
            map.put("height", height);
            // For Android Texture rendering with ZegoSurfaceTextureRenderer, attach textureID if available
            Long textureID = ZegoTextureRendererController.getInstance().mediaPlayerTextureInUse.get(mediaPlayer.getIndex());
            if (textureID != null) {
                map.put("textureID", textureID.intValue());
            }
    ...
    IZegoAudioDataHandler audioDataHandler = new IZegoAudioDataHandler() {
        ...
        public void onCapturedAudioData(ByteBuffer data, int dataLength, ZegoAudioFrameParam param){
            ...
            IZegoAudioDataHandler appHandler = ZegoAudioDataManager.getInstance().getHandler();
            if (appHandler != null) {
                appHandler.onCapturedAudioData(ByteBuffer.wrap(bytes), dataLength, param);
            }
        ...
        public void onPlaybackAudioData(ByteBuffer data, int dataLength, ZegoAudioFrameParam param){
            ...
            IZegoAudioDataHandler appHandler = ZegoAudioDataManager.getInstance().getHandler();
            if (appHandler != null) {
                appHandler.onPlaybackAudioData(ByteBuffer.wrap(bytes), dataLength, param);
            }
        ...
        public void onMixedAudioData(ByteBuffer data, int dataLength, ZegoAudioFrameParam param){
            ...
            IZegoAudioDataHandler appHandler = ZegoAudioDataManager.getInstance().getHandler();
            if (appHandler != null) {
                appHandler.onMixedAudioData(ByteBuffer.wrap(bytes), dataLength, param);
            }
        ...
        public void onPlayerAudioData(ByteBuffer data, int dataLength, ZegoAudioFrameParam param, String streamID){
            ...
            IZegoAudioDataHandler appHandler = ZegoAudioDataManager.getInstance().getHandler();
            if (appHandler != null) {
                appHandler.onPlayerAudioData(ByteBuffer.wrap(bytes), dataLength, param, streamID);
            }
    ...
    IZegoCustomAudioProcessHandler customAudioProcessHandler = new IZegoCustomAudioProcessHandler() {
        ...
        public void onProcessCapturedAudioData(ByteBuffer data, int dataLength, ZegoAudioFrameParam param, double timestamp) {
            ...
            if (ZegoCustomAudioProcessManager.getInstance().getHandler() != null) {
                ...
                ZegoCustomAudioProcessManager.getInstance().getHandler().onProcessCapturedAudioData(data, dataLength, audioFrameParam, timestamp);
                param.sampleRate = ZegoAudioSampleRate.getZegoAudioSampleRate(audioFrameParam.sampleRate.value());
                param.channel = ZegoAudioChannel.getZegoAudioChannel(audioFrameParam.channel.value());
            }
        }
        ...
        public void onProcessCapturedAudioDataAfterUsedHeadphoneMonitor(ByteBuffer data, int dataLength, ZegoAudioFrameParam param, double timestamp) {
            ...
            if (ZegoCustomAudioProcessManager.getInstance().getHandler() != null) {
                ...
                audioFrameParam.sampleRate = ZGFlutterAudioSampleRate.getZGFlutterAudioSampleRate(param.sampleRate.value());
                audioFrameParam.channel = ZGFlutterAudioChannel.getZGFlutterAudioChannel(param.channel.value());
                ZegoCustomAudioProcessManager.getInstance().getHandler().onProcessCapturedAudioDataAfterUsedHeadphoneMonitor(data, dataLength, audioFrameParam, timestamp);
                param.sampleRate = ZegoAudioSampleRate.getZegoAudioSampleRate(audioFrameParam.sampleRate.value());
                param.channel = ZegoAudioChannel.getZegoAudioChannel(audioFrameParam.channel.value());
            }
        }
        ...
        @Override
        public void onBeforeAudioPrepAudioData(ByteBuffer data, int dataLength,
            ZegoAudioFrameParam param) {
            super.onBeforeAudioPrepAudioData(data, dataLength, param);
            ...
            map.put("method", "onBeforeAudioPrepAudioData");
            map.put("data", bytes);
            map.put("dataLength", dataLength);
            map.put("param", paramMap);
            ...
            if (ZegoCustomAudioProcessManager.getInstance().getHandler() != null) {
                ...
                audioFrameParam.sampleRate = ZGFlutterAudioSampleRate.getZGFlutterAudioSampleRate(param.sampleRate.value());
                audioFrameParam.channel = ZGFlutterAudioChannel.getZGFlutterAudioChannel(param.channel.value());
                ZegoCustomAudioProcessManager.getInstance().getHandler().onBeforeAudioPrepAudioData(data, dataLength, audioFrameParam);
                param.sampleRate = ZegoAudioSampleRate.getZegoAudioSampleRate(audioFrameParam.sampleRate.value());
                param.channel = ZegoAudioChannel.getZegoAudioChannel(audioFrameParam.channel.value());
            }
        }
        ...
        public void onAlignedAudioAuxData(ByteBuffer data,int dataLength,ZegoAudioFrameParam param) {
            ...
            if (ZegoCustomAudioProcessManager.getInstance().getHandler() != null) {
                ...
                audioFrameParam.sampleRate = ZGFlutterAudioSampleRate.getZGFlutterAudioSampleRate(param.sampleRate.value());
                audioFrameParam.channel = ZGFlutterAudioChannel.getZGFlutterAudioChannel(param.channel.value());
                ZegoCustomAudioProcessManager.getInstance().getHandler().onAlignedAudioAuxData(data, dataLength, audioFrameParam);
                param.sampleRate = ZegoAudioSampleRate.getZegoAudioSampleRate(audioFrameParam.sampleRate.value());
                param.channel = ZegoAudioChannel.getZegoAudioChannel(audioFrameParam.channel.value());
            }
        }
        ...
        public void onProcessRemoteAudioData(ByteBuffer data, int dataLength, ZegoAudioFrameParam param, String streamID, double timestamp) {
            ...
            if (ZegoCustomAudioProcessManager.getInstance().getHandler() != null) {
                ...
                audioFrameParam.sampleRate = ZGFlutterAudioSampleRate.getZGFlutterAudioSampleRate(param.sampleRate.value());
                audioFrameParam.channel = ZGFlutterAudioChannel.getZGFlutterAudioChannel(param.channel.value());
                ZegoCustomAudioProcessManager.getInstance().getHandler().onProcessRemoteAudioData(data, dataLength, audioFrameParam, streamID, timestamp);
                param.sampleRate = ZegoAudioSampleRate.getZegoAudioSampleRate(audioFrameParam.sampleRate.value());
                param.channel = ZegoAudioChannel.getZegoAudioChannel(audioFrameParam.channel.value());
            }
        }
        ...
        public void onProcessPlaybackAudioData(ByteBuffer data, int dataLength, ZegoAudioFrameParam param, double timestamp) {
            ...
            if (ZegoCustomAudioProcessManager.getInstance().getHandler() != null) {
                ...
                audioFrameParam.sampleRate = ZGFlutterAudioSampleRate.getZGFlutterAudioSampleRate(param.sampleRate.value());
                audioFrameParam.channel = ZGFlutterAudioChannel.getZGFlutterAudioChannel(param.channel.value());
                ZegoCustomAudioProcessManager.getInstance().getHandler().onProcessPlaybackAudioData(data, dataLength, audioFrameParam, timestamp);
                param.sampleRate = ZegoAudioSampleRate.getZegoAudioSampleRate(audioFrameParam.sampleRate.value());
                param.channel = ZegoAudioChannel.getZegoAudioChannel(audioFrameParam.channel.value());
            }
        }
